...
P2PPong.sendGameAction = async function(channelId, action, gameData) {
...
// ===== Обработка игровых действий =====
P2PPong._handleGameAction = async function(channelId, data) {
...
P2PPong.getGameState = function() {
    return this._gameState;
};

// ===== Обновление игрового состояния (локальное) =====
P2PPong.updateGameState = function(state) {
    Object.assign(this._gameState, state);
    this._emit('game-state-updated', this._gameState);
};

// ===== Добавляем обработку игровых сообщений в _handleIn =====
// НАЙДИТЕ В ФУНКЦИИ _handleIn МЕСТО ГДЕ ОБРАБАТЫВАЕТСЯ d.type
// И ДОБАВЬТЕ ЭТОТ КЕЙС:

// В _handleIn после обработки d.type === 'verification-code' добавьте:
if (d.type === 'game') {
    this._handleGameAction(this._chId, d);
    return;
}
