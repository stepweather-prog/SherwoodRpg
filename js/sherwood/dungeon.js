Sherwood.Dungeon = {
    ...
    getAvailable: function() {
        ...
        for (var id in duns) {
            ...
            if (dd.requiredDungeon) {
                ...
                if (lastLevel < 8 || lastCups < dd.requiredCups) {
                    isUnlocked = false;
                }
            }
            list[id] = { 
    ...
    generate: function(dungeonId, level) {
        ...
        var specials = [
            { type: self.TILE.POTION, count: 5, prop: 'potion' },
            { type: self.TILE.CAULDRON, count: 4, prop: 'cauldron' },
            { type: self.TILE.ALTAR, count: 4, prop: 'altar' }
        ];
