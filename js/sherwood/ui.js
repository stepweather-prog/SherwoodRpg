...
_doStep: function(tx, ty) {
    ...
    if (res.type === 'exit') { this._stopSound('steps'); this._stopMusic(); var reward = Sherwood.Dungeon.complete(); SherwoodUI.updateDisplay(); this._afterRewardAction = function() { SherwoodUI._playMusic('main_theme'); SherwoodUI.showDungeon(); }; this._showVictoryScreen({ exp: reward.exp, gold: reward.gold, silver: reward.silver }); }
    if (res.type === 'exit_locked') { this._stopSound('steps'); this._showToast('Закрыто! Убейте всех монстров!'); }
...
_showBattleScreen: function(enemyData, mode, modeTitle, extraInfo, onAttack, onFlee) {
    ...
    // Враг: аватар + название + HP
    h += '<div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-bottom:2px;">';
    ...
    h += '<span style="color:#f44336;font-weight:bold;font-size:0.85em;">' + e.name + '</span>';
    h += '<div style="position:relative;width:150px;height:20px;">';
    ...
    h += '</div>';
    h += '<span id="enemy-hp-text" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-size:0.5em;z-index:2;text-shadow:0 0 4px #000;">' + e.hp + '/' + e.maxHp + '</span></div>';
    h += '</div>';
    
    // Карточка бестии
    h += '<div style="margin:2px 0;position:relative;display:inline-block;" id="enemy-card-area">';
    ...
    for (var i = 0; i < Math.min(2, unlockedSkills.length); i++) {
        ...
        h += '<button onclick="SherwoodUI._useSkill(\'' + sk.id + '\')" style="background:rgba(201,168,76,0.2);border:1px solid #c9a040;border-radius:50%;width:40px;height:40px;cursor:pointer;padding:2px;position:relative;"><img src="' + sk.icon + '" style="width:100%;height:100%;object-fit:contain;border-radius:50%;"></button>';
    }
    h += '<button onclick="' + onAttack + '" style="background:url(assets/skills/skill_shot_normal.png) center/contain no-repeat;width:52px;height:52px;border:3px solid #c9a040;border-radius:50%;cursor:pointer;"></button>';
    ...
    h += '</div>';
    h += '<div style="position:relative;width:190px;height:20px;">';
    h += '<img src="assets/interface/life_scale.png" style="width:100%;height:100%;position:absolute;top:0;left:0;z-index:0;">';
    h += '<div style="position:absolute;top:2px;left:2px;right:2px;bottom:2px;overflow:hidden;z-index:1;">';
    ...
    h += '</div>';
    h += '<span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-size:0.5em;z-index:2;text-shadow:0 0 4px #000;">' + p.stats.hp + '/' + p.stats.maxHp + '</span></div>';
    h += '</div>';
    
    // Лог боя
    h += '<div id="battle-dialog" style="background:rgba(0,0,0,0.75);border:1px solid #555;border-radius:8px;padding:4px;margin:2px 4%;min-height:45px;max-height:45px;overflow-y:auto;color:#aaa;font-size:0.6em;text-align:left;line-height:1.3;"></div>';
    h += '</div>';
...
portal: function() {
    ...
    if (portals && Object.keys(portals).length > 0) {
        ...
        for (var id in portals) {
            ...
            var badge = completed ? '<span style="color:#4caf50;">Пройдено</span>' : '<span style="color:#ffd700;">Доступен</span>';
            var bo = completed ? '0.3' : '0.6';
            var bc = completed ? '#4caf50' : '#c9a040';
            var ca = completed ? '' : 'onclick="SherwoodUI._enterPortal(' + p.id + ')" style="cursor:pointer;"';
        ...
        h = '<div style="color:#aaa;text-align:center;padding:40px;">Нет доступных порталов</div>';
    }
    try { if (this._mainElements) this._mainElements.forEach(function(sel) { document.querySelectorAll(sel).forEach(function(el) { el.style.display = 'none'; }); }); } catch(e) {}
    try { this.container.style.background = "url('" + (this._bg['portal'] || 'assets/backgrounds/portal_1.jpeg') + "') center/cover no-repeat"; } catch(e) {}
    try { if (this._screenLayer) { this._screenLayer.innerHTML = '<div style="min-height:100%;padding:16px;display:flex;flex-direction:column;overflow-y:auto;"><div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;"><button onclick="SherwoodUI.loadHome()" style="background:transparent;border:none;cursor:pointer;padding:0;width:50px;height:50px;"><img src="assets/all_buttons/back.png" style="width:100%;height:100%;object-fit:contain;"></button><span style="color:#e0c080;font-size:1.1em;">Порталы</span></div><div style="flex:1;overflow-y:auto;">' + h + '</div></div>'; this._screenLayer.style.display = 'block'; } } catch(e) {}
},
...
h += '<div class="profile-action-btn" onclick="SherwoodUI._showAllTrophies()"><img src="assets/all_trophies/trophies_chapters/chapter_1_broken_hunting_horn_of_the_league.png"><span class="action-label">' + (trophies.length > 0 ? trophies.length + ' трофеев' : 'Трофеи') + '</span></div>';
