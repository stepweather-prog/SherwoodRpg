...
forge: function() {
    ...
    h += '<div style="color:#aaa;font-size:0.7em;">Уровень: ' + ringInfo.currentLevel + '/' + ringInfo.maxLevel + ' | Скрижалей: ' + ringInfo.tablets + '</div>';
    if (ringInfo.canCraft) { h += '<button onclick="SherwoodUI._craftRing()" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Улучшить (' + ringInfo.cost + ' скрижалей)</button>'; }
    else { h += '<div style="color:#888;font-size:0.6em;">' + (ringInfo.reason || 'Нужно ' + ringInfo.cost + ' скрижалей') + '</div>'; }
    h += '</div>';
    h += '<div style="margin-bottom:12px;"><div style="color:#e0c080;margin-bottom:4px;">Амулеты</div>';
    h += '<div style="color:#aaa;font-size:0.7em;">Уровень: ' + amuletInfo.currentLevel + '/' + amuletInfo.maxLevel + ' | Скрижалей: ' + amuletInfo.tablets + '</div>';
    if (amuletInfo.canCraft) { h += '<button onclick="SherwoodUI._craftAmulet()" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Улучшить (' + amuletInfo.cost + ' скрижалей)</button>'; }
    else { h += '<div style="color:#888;font-size:0.6em;">' + (amuletInfo.reason || 'Нужно ' + amuletInfo.cost + ' скрижалей') + '</div>'; }
    h += '</div>';
