...
_startDungeon: function(id, level) { 
    ...
    for (var y = 0; y < size; y++) {
        for (var x = 0; x < size; x++) {
            ...
            var content = "";
            if (!isPlayer) {
                if (cell.open && cell.monster) content = "<img src='assets/all_beasts/" + (cell.monsterId || "plague_crow.png") + "' style='width:90%;height:90%;object-fit:contain;'>";
...
_showBattleScreen: function(enemyData, mode, modeTitle, extraInfo, onAttack, onFlee, customBg) {
    ...
    // Кнопка назад
    h += '<div style="display:flex;align-items:center;gap:8px;width:100%;margin-bottom:2px;">';
    h += '<button onclick="' + onFlee + '" style="background:transparent;border:none;cursor:pointer;padding:0;width:32px;height:32px;flex-shrink:0;"><img src="assets/all_buttons/back.png" style="width:100%;height:100%;object-fit:contain;"></button>';
    h += '<span style="color:#e0c080;font-size:0.8em;flex:1;text-align:center;">' + (modeTitle || '') + '</span>';
    h += '<div style="width:32px;height:32px;"></div>';
    h += '</div>';
    ...
    // Статы врага
    h += '<div style="display:flex;gap:10px;justify-content:center;margin-bottom:2px;">';
    h += '<span style="color:#fff;font-size:0.75em;font-weight:bold;">АТК ' + (e.attack || 0) + '</span>';
    h += '<span style="color:#fff;font-size:0.75em;font-weight:bold;">ЗЩТ ' + (e.defense || 0) + '</span>';
    h += '<span style="color:#fff;font-size:0.75em;font-weight:bold;">HP ' + e.hp + '</span>';
    h += '</div>';
    
    // Карта врага
    h += '<div style="position:relative;display:inline-block;" id="enemy-card-area">';
    h += '<img src="' + imgPath + '" id="enemy-card" style="width:240px;height:240px;object-fit:contain;position:relative;z-index:1;" onerror="this.style.display=&quot;none&quot;">';
    ...
    if (Sherwood.Combat && Sherwood.Combat._battle) {
        var battle = Sherwood.Combat._battle;
        if (battle.chargedSkills && battle.chargedSkills.length > 0) {
            chargedSkillLeft = battle.chargedSkills[0] || null;
            chargedSkillRight = battle.chargedSkills[1] || null;
        }
    }
    var skills = Sherwood.Combat ? Sherwood.Combat.getSkills() : {};
    ...
    h += '<span style="color:#fff;font-size:0.75em;font-weight:bold;">HP ' + p.stats.hp + '</span>';
    h += '</div>';
...
tavern: function() {
    ...
    // Рамка с егерем по центру сверху
    h += '<div style="display:flex;flex-direction:column;align-items:center;padding:20px;">';
    
    h += '<div style="position:relative;width:240px;height:240px;margin-bottom:16px;">';
    ...
    h += '<button onclick="Sherwood.Tavern._tab=2;SherwoodUI.tavern();" style="background:' + (tab === 2 ? '#c9a040' : 'rgba(255,255,255,0.1)') + ';border:1px solid #555;border-radius:8px;padding:10px 24px;color:' + (tab === 2 ? '#000' : '#fff') + ';cursor:pointer;font-size:0.85em;font-weight:bold;">Таланты</button>';
    h += '</div>';
    ...
    if (tab === 1) {
        ...
        if (isActive) {
            var q = active.quest;
            h += '<div style="position:relative;width:100%;max-width:440px;min-height:260px;display:flex;align-items:center;justify-content:center;">';
            h += '<img src="assets/interface/contracts.png" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:fill;">';
            h += '<div style="position:relative;z-index:1;text-align:center;padding:28px 20px;width:100%;">';
            h += '<div style="color:#000;font-size:1.3em;font-weight:900;margin-bottom:10px;">' + q.name + '</div>';
            h += '<div style="color:#000;font-size:1em;font-weight:900;margin-bottom:8px;">' + q.desc + '</div>';
            h += '<div style="color:#8b0000;font-size:0.9em;font-weight:900;">Противник: ' + q.enemy.name + ' (HP ' + q.enemy.hp + ')</div>';
            h += '</div>';
            h += '</div>';
            
            h += '<button onclick="SherwoodUI._tavernBattle()" style="margin-top:12px;background:#c9a040;border:none;border-radius:10px;padding:14px 40px;color:#000;font-weight:900;cursor:pointer;font-size:1em;">В бой</button>';
        } else if (cooldown) {
            h += '<div style="position:relative;width:100%;max-width:440px;min-height:260px;display:flex;align-items:center;justify-content:center;">';
            h += '<img src="assets/interface/contracts.png" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:fill;">';
            h += '<div style="position:relative;z-index:1;text-align:center;padding:28px 20px;width:100%;">';
            h += '<div style="color:#000;font-size:1.3em;font-weight:900;margin-bottom:10px;">Перезарядка: ' + cdRemain + ' мин.</div>';
            h += '<div style="color:#000;font-size:1em;font-weight:900;">Контрактов сегодня: ' + dailyDone + '/' + dailyMax + '</div>';
            h += '</div>';
            h += '</div>';
        } else {
            ...
            h += '<img src="assets/interface/contracts.png" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:fill;">';
            h += '<div style="position:relative;z-index:1;text-align:center;padding:28px 20px;width:100%;">';
            h += '<div style="color:#000;font-size:1.3em;font-weight:900;margin-bottom:10px;">' + nextQuest.name + '</div>';
            h += '<div style="color:#000;font-size:1em;font-weight:900;margin-bottom:8px;">' + nextQuest.desc + '</div>';
            h += '<div style="color:#8b0000;font-size:0.9em;font-weight:900;margin-bottom:8px;">Противник: ' + nextQuest.enemy.name + ' (HP ' + nextQuest.enemy.hp + ')</div>';
            h += '<div style="color:#000;font-size:0.9em;font-weight:900;">+' + nextQuest.reward.exp + 'XP +' + nextQuest.reward.gold + 'G</div>';
            h += '</div>';
            h += '</div>';
            
            if (dailyDone < dailyMax && completedCount < 100) {
                h += '<button onclick="SherwoodUI._tavernStart(0,0)" style="margin-top:12px;background:#c9a040;border:none;border-radius:10px;padding:14px 40px;color:#000;font-weight:900;cursor:pointer;font-size:1em;">В бой</button>';
            }
        }
        ...
        // ТАЛАНТЫ
        var skills = Sherwood.Tavern.getTalents();
        
        h += '<div style="width:100%;">';
        h += '<div style="color:#e0c080;font-size:1em;font-weight:bold;text-align:center;margin-bottom:12px;">Таланты</div>';
...
_showPortalBattle: function() {
    ...
    var imgPath = 'assets/portal_beasts/' + enemy.image;
    ...
    h += '<img src="' + imgPath + '" style="width:160px;height:160px;object-fit:contain;margin-bottom:8px;" onerror="this.src=\'assets/interface/labyrinth_of_icons.png\'">';
...
raid: function() { 
    ...
    for (var i = 0; i < raids.length; i++) {
        ...
        html += '<img src="assets/portal_beasts/the_primordial_core.png" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:180px;height:180px;object-fit:contain;" onerror="this.src=\'assets/interface/labyrinth_of_icons.png\'">';
...
_changePlayerName: function() { 
    ...
    if (p.nameChanges === 0) {
        ...
        p.nameChanges = 1;
        Sherwood.saveGame();
        if (Sherwood.Chat) Sherwood.Chat.setUsername(nm);
...
_addWalletSilver: function(amount) {
    ...
    h += '<div style="text-align:center;margin-bottom:12px;">';
    h += '<div onclick="SherwoodUI._showSkinSelector()" style="cursor:pointer;display:inline-block;position:relative;">';
    h += '<img src="assets/hero_skins/' + activeSkin + '.png" style="width:180px;height:180px;border-radius:16px;border:3px solid #ffd700;object-fit:contain;" onerror="this.src=\'assets/hero_skins/skin1_01.png\'">';
    h += '<div style="position:absolute;bottom:-6px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.8);color:#ffd700;font-size:0.6em;font-weight:bold;padding:3px 10px;border-radius:4px;white-space:nowrap;">Сменить</div>';
    h += '</div>';
    h += '<div style="color:#e0c080;font-weight:bold;margin-top:10px;font-size:1.2em;">' + p.name + '</div>';
    h += '<div style="color:#aaa;font-size:1em;">Уровень ' + p.level + '</div></div>';

    // Кнопки: Трофеи, Кольца, Амулеты, Таланты
...
_showTalentsFromProfile: function() {
    ...
    h += '<div style="color:#e0c080;font-size:1.1em;font-weight:bold;text-align:center;margin-bottom:12px;">Мои таланты</div>';
    ...
    for (var id in skills) {
        ...
        h += '<div onclick="SherwoodUI._toggleTalentFromProfile(\'' + id + '\')" style="cursor:pointer;background:rgba(0,0,0,0.5);border:2px solid ' + (isActive ? '#4caf50' : '#555') + ';border-radius:8px;padding:10px;margin-bottom:8px;display:flex;align-items:center;gap:10px;">';
        h += '<img src="' + s.icon + '" style="width:44px;height:44px;object-fit:contain;">';
        h += '<div style="flex:1;"><div style="color:#e0c080;">' + s.name + '</div><div style="color:#aaa;font-size:0.7em;">' + s.description + '</div></div>';
        
        if (isActive) {
            h += '<div style="color:#4caf50;font-size:0.7em;font-weight:bold;">Вкл</div>';
        } else {
            h += '<div style="color:#888;font-size:0.7em;font-weight:bold;">Выкл</div>';
        }
    ...
    if (!hasUnlocked) {
        h += '<div style="color:#aaa;text-align:center;padding:20px;">Нет изученных талантов</div>';
    }
    
    h += '<div style="color:#aaa;font-size:0.65em;text-align:center;margin-top:8px;">Нажми на талант чтобы включить или выключить</div>';
    h += '</div>';
...
_showSkinSelector: function() {
    ...
    for (var i = 0; i < unlockedSkins.length; i++) {
        ...
        h += '<div onclick="' + (isActive ? '' : 'SherwoodUI._selectSkin(\'' + sid + '\')') + '" style="cursor:' + (isActive ? 'default' : 'pointer') + ';background:rgba(0,0,0,0.5);border:2px solid ' + (isActive ? '#ffd700' : '#555') + ';border-radius:8px;padding:10px;width:80%;text-align:center;">';
...
forge: function() {
    ...
    // Меню с 4 кнопками-ячейками
    h += '<div style="display:flex;gap:8px;justify-content:center;margin-bottom:16px;">';
    
    // Чертёж скина
    h += '<div onclick="SherwoodUI._showSkinCrafting()" style="cursor:pointer;position:relative;width:70px;height:70px;background:url(\'assets/interface/bag_cell.png\') center/contain no-repeat;background-size:cover;border:2px solid #c9a040;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;">';
    h += '<img src="assets/interface/skin_drawing.png" style="width:44px;height:44px;object-fit:contain;">';
    h += '<span style="position:absolute;bottom:2px;right:4px;color:#fff;font-size:0.6em;font-weight:bold;background:rgba(0,0,0,0.8);padding:1px 6px;border-radius:4px;">' + skinDrawings + '</span>';
    h += '</div>';
    
    // Скрижаль колец
    h += '<div onclick="SherwoodUI._showRingCrafting()" style="cursor:pointer;position:relative;width:70px;height:70px;background:url(\'assets/interface/bag_cell.png\') center/contain no-repeat;background-size:cover;border:2px solid #c9a040;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;">';
    h += '<img src="assets/interface/ring_crafting_tablet_resource.png" style="width:44px;height:44px;object-fit:contain;">';
    h += '<span style="position:absolute;bottom:2px;right:4px;color:#fff;font-size:0.6em;font-weight:bold;background:rgba(0,0,0,0.8);padding:1px 6px;border-radius:4px;">' + ringTablets + '</span>';
    h += '</div>';
    
    // Скрижаль амулетов
    h += '<div onclick="SherwoodUI._showAmuletCrafting()" style="cursor:pointer;position:relative;width:70px;height:70px;background:url(\'assets/interface/bag_cell.png\') center/contain no-repeat;background-size:cover;border:2px solid #c9a040;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;">';
    h += '<img src="assets/interface/amulet_crafting_tablet_resource.png" style="width:44px;height:44px;object-fit:contain;">';
    h += '<span style="position:absolute;bottom:2px;right:4px;color:#fff;font-size:0.6em;font-weight:bold;background:rgba(0,0,0,0.8);padding:1px 6px;border-radius:4px;">' + amuletTablets + '</span>';
    h += '</div>';
    ...
    if (enhanceItems.length > 0) {
        for (var i = 0; i < enhanceItems.length; i++) {
            ...
            h += '<button onclick="SherwoodUI._enhanceItem(' + idx + ')" style="background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Точить</button>';
            h += '</div>';
        }
    } else {
        h += '<div style="color:#aaa;font-size:0.7em;">Нет предметов для заточки</div>';
    }
    h += '</div>';
...
_showSkinCrafting: function() {
    ...
    var skinDrawings = resources.skinTablets || 0;
    var skinScrolls = resources.skinTablets || 0;
    
    var h = '<div style="text-align:center;color:#e0c080;font-size:1.1em;font-weight:bold;margin-bottom:12px;">Создание обликов</div>';
    h += '<div style="color:#aaa;font-size:0.7em;margin-bottom:8px;">Чертежей: ' + skinDrawings + ' | Скрижалей: ' + skinScrolls + '</div>';
    ...
    for (var i = 0; i < skins.length; i++) {
        ...
        if (owned) {
            ...
            h += '<button onclick="SherwoodUI._craftSkin(\'' + skin.id + '\')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.7em;font-weight:bold;">Создать</button>';
        }
    ...
    var gb = 'SherwoodUI.forge()';
    this._openScreenScrollable('Облики', 'forge', h, gb);
},
...
_showRingCrafting: function() {
    ...
    for (var i = 0; i < rings.length; i++) {
        ...
        if (owned) {
            ...
            h += '<button onclick="SherwoodUI._craftRingFromForge(\'' + ring.id + '\')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.7em;font-weight:bold;">Создать</button>';
        }
...
_showAmuletCrafting: function() {
    ...
    for (var i = 0; i < amulets.length; i++) {
        ...
        if (owned) {
            ...
            h += '<div style="color:#aaa;font-size:0.65em;margin-top:4px;">Скрижалей: 10</div>';
            h += '<button onclick="SherwoodUI._craftAmuletFromForge(\'' + amulet.id + '\')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.7em;font-weight:bold;">Создать</button>';
        }
...
_craftRingFromForge: function(ringId) {
    var r = Sherwood.Forge.craftRing ? Sherwood.Forge.craftRing(ringId) : { success: false, reason: 'Нет доступа' };
    var info = document.getElementById('forge-info');
    if (!info) info = document.createElement('div');
    
    if (r.success) {
        this._showToast('Кольцо выковано!');
    } else {
        this._showToast(r.reason || 'Ошибка');
    }
...
_craftAmuletFromForge: function(amuletId) {
    var r = Sherwood.Forge.craftAmulet ? Sherwood.Forge.craftAmulet(amuletId) : { success: false, reason: 'Нет доступа' };
    
    if (r.success) {
        this._showToast('Амулет выкован!');
    } else {
        this._showToast(r.reason || 'Ошибка');
    }
...
bestiary: function() { 
    ...
    for (var i=0; i<beasts.length; i++) {
        ...
        h += '<img src="' + beastImgPath + '" style="width:60px;height:60px;object-fit:contain;border-radius:8px;'+(disc?'':'filter:grayscale(1);opacity:0.5;')+'" onerror="this.src=\'assets/interface/labyrinth_of_icons.png\'">';
...
_showBeastInfo: function(beastId) { 
    ...
    h += '<img src="' + beastImgPath + '" style="width:140px;height:140px;object-fit:contain;border:2px solid #c9a040;border-radius:10px;'+(disc?'':'filter:grayscale(1);opacity:0.5;')+'" onerror="this.src=\'assets/interface/labyrinth_of_icons.png\'">';
