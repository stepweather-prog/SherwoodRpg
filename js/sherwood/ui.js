    ...
    try {
        ...
        if (silverEl) {
            ...
            silverEl.parentElement.onclick = function() {
                var amount = prompt('Exchange gold to silver.\nRate: 1 gold = 100 silver.\nHow much gold?', '1');
...
_playMusic: function(k) {
    try {
        ...
        if (k === 'main_theme' && Sherwood.Dungeon && Sherwood.Dungeon.getDungeon && Sherwood.Dungeon.getDungeon()) return;
...
loadHome: function() {
    ...
    _showPlaceholder: function(title, bgKey, backAction) { this._playSound('click'); this._openScreen(title, bgKey, '<div style="text-align:center;padding:40px 0;"><div style="font-size:3em;margin-bottom:16px;">&#128679;</div><div style="font-size:1.2em;color:#e0c080;margin-bottom:8px;">'+title+'</div><div style="font-size:0.7em;color:#888;">In development</div></div>', backAction); },

    _showVictoryScreen: function(rewards) { var h = '<div style="text-align:center;padding:10px;"><div style="position:relative;display:inline-block;"><img src="assets/interface/vertical_slab_victory.png" style="width:300px;height:auto;display:block;"><div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;width:80%;"><div style="color:#ffd700;font-size:1.1em;font-weight:bold;">VICTORY!</div>'; if (rewards.exp) h += '<div style="color:#fff;font-size:0.85em;">+ ' + rewards.exp + ' XP</div>'; if (rewards.gold) h += '<div style="color:#ffd700;">+ ' + rewards.gold + ' Gold</div>'; if (rewards.silver) h += '<div style="color:#c0c0c0;">+ ' + rewards.silver + ' Silver</div>'; if (rewards.scrolls) h += '<div style="color:#9c27b0;">+ ' + rewards.scrolls + ' Scrolls</div>'; if (rewards.ingots) h += '<div style="color:#ff9800;">+ ' + rewards.ingots + ' Ingots</div>'; h += '</div><button onclick="SherwoodUI._claimReward()" style="position:absolute;bottom:20px;left:50%;transform:translateX(-50%);background:#c9a040;border:none;border-radius:8px;padding:8px 24px;color:#000;font-weight:bold;cursor:pointer;font-size:0.8em;z-index:2;">Claim</button></div></div>'; this._openScreen('Victory', 'dungeon_fight', h); },
    _showDefeatScreen: function(rewards) { var h = '<div style="text-align:center;padding:10px;"><div style="position:relative;display:inline-block;"><img src="assets/interface/vertical_slab_defeat.png" style="width:300px;height:auto;display:block;"><div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;width:80%;"><div style="color:#f44336;font-size:1.1em;font-weight:bold;">DEFEAT</div>'; if (rewards.exp) h += '<div style="color:#fff;font-size:0.85em;">+ ' + rewards.exp + ' XP</div>'; if (rewards.silver) h += '<div style="color:#c0c0c0;">+ ' + rewards.silver + ' Silver</div>'; if (rewards.scrolls) h += '<div style="color:#9c27b0;">+ ' + rewards.scrolls + ' Scrolls</div>'; h += '</div><button onclick="SherwoodUI._claimReward()" style="position:absolute;bottom:20px;left:50%;transform:translateX(-50%);background:#c9a040;border:none;border-radius:8px;padding:8px 24px;color:#000;font-weight:bold;cursor:pointer;font-size:0.8em;z-index:2;">Claim</button></div></div>'; this._openScreen('Defeat', 'dungeon_fight', h); },
    ...
    var wood = 0;
    try { var bag = Sherwood.Bag.getItems(); for (var i=0;i<bag.length;i++) { if (bag[i].id === 'wood' || bag[i].name === 'Wood') wood += bag[i].quantity || 0; } } catch(e) {}
    var canActivate = wood >= 100 && now > cooldown;
    var h = '<div style="text-align:center;padding:20px;"><div style="font-size:3em;">&#128293;</div><div style="color:#e0c080;font-size:1.2em;margin:12px 0;">Hearth of Sherwood</div><div style="color:#aaa;font-size:0.85em;">Wood in bag: ' + wood + ' / 100</div>';
    if (bonusActive && now < bonusEnd) { var rem = Math.ceil((bonusEnd - now)/3600000); h += '<div style="color:#4caf50;margin:12px 0;">Bonus +20% active! Left: ' + rem + ' h.</div>'; }
    else if (canActivate) { h += '<button onclick="SherwoodUI._activateHearth()" style="margin-top:12px;background:#c9a040;border:none;border-radius:8px;padding:10px 24px;color:#000;font-weight:bold;cursor:pointer;">Add wood (100)</button>'; }
    else if (now <= cooldown) { var cd = Math.ceil((cooldown - now)/3600000); h += '<div style="color:#ff9800;margin:12px 0;">Cooldown: ' + cd + ' h.</div>'; }
    else { h += '<div style="color:#f44336;margin:12px 0;">Not enough wood (need 100)</div>'; }
    h += '</div>';
    this._openScreen('Hearth', 'hearth', h);
},

_activateHearth: function() {
    var p = Sherwood.getPlayer();
    var spent = 0;
    try { var bag = Sherwood.Bag.getItems(); for (var i=bag.length-1;i>=0&&spent<100;i--) { if (bag[i].id==='wood'||bag[i].name==='Wood') { var q=bag[i].quantity||1; var t=Math.min(q,100-spent); spent+=t; if(q<=t) bag.splice(i,1); else bag[i].quantity-=t; } } Sherwood.Bag._save(); } catch(e) {}
...
talents: function() {
    ...
    var h = '<div style="padding:10px;"><div style="color:#e0c080;font-size:1.1em;text-align:center;margin-bottom:12px;">Talents</div>';
    ...
    if (s.unlocked) h+='<div style="color:#4caf50;font-size:0.7em;">Unlocked</div>';
    else h+='<button onclick="SherwoodUI._unlockTalent(\''+id+'\')" style="background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Learn ('+s.cost+' Gold)</button>';
    h+='</div>'; }
    h += '</div>';
    this._openScreen('Talents', 'talents', h);
},

_unlockTalent: function(id) { if(!Sherwood.Combat||!Sherwood.Combat.unlockSkill) return; var r=Sherwood.Combat.unlockSkill(id); if(r.success){this.updateDisplay();this.talents();} else this._showNotification(r.reason||'Error'); },
    ...
    showDungeon: function() {
        ...
        for (var id in dungeons) {
            ...
            list += '</div><div style="text-align:center;color:#666;font-size:0.7em;margin-top:6px;">' + ((progress.level||1) >= 7 ? 'Completed' : 'Next: ' + ((progress.level||1) + 1)) + '</div></div></div>';
        }
        var tickets = Sherwood.getPlayer() ? Sherwood.getPlayer().dungeon.tickets : 0, html = '<div style="color:#aaa;font-size:0.85em;margin-bottom:12px;text-align:center;">Tickets: ' + tickets + '</div>' + (list || '<div style="color:#aaa;text-align:center;">No dungeons</div>');
        try { if (this._mainElements) this._mainElements.forEach(function(sel) { document.querySelectorAll(sel).forEach(function(el) { el.style.display = 'none'; }); }); } catch(e) {}
        this.container.style.background = "url('" + this._bg.dungeon_select + "') center/cover no-repeat";
        if (this._screenLayer) { this._screenLayer.innerHTML = '<div style="min-height:100%;background:rgba(0,0,0,0.7);padding:16px;display:flex;flex-direction:column;"><div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;"><button onclick="SherwoodUI.loadHome()" style="background:transparent;border:none;cursor:pointer;padding:0;width:50px;height:50px;"><img src="assets/all_buttons/back.png" style="width:100%;height:100%;object-fit:contain;"></button><span style="color:#e0c080;font-size:1.1em;">Dungeons</span></div><div style="flex:1;">' + html + '</div></div>'; this._screenLayer.style.display = 'block'; }
    },
    _startDungeon: function(id, level) { if (!Sherwood.Dungeon || !Sherwood.Dungeon.generate) return; var d = Sherwood.Dungeon.generate(id, level); if (!d) { this._showNotification('No tickets!'); return; } this._playSound('dungeon_enter'); this._renderDungeon(); },
    ...
    for (var y = 0; y < size; y++) {
        for (var x = 0; x < size; x++) {
            ...
            if (!isPlayer) {
                ...
                else if (cell.open && cell.exit) content = cell.locked ? "<img src='assets/interface/closed_level_lock_icon.png' style='width:80%;height:80%;object-fit:contain;'>" : "<img src='assets/interface/exit_completion_dungeon.png' style='width:80%;height:80%;object-fit:contain;'>";
            }
            if (isPlayer) {
                if (d.isMoving) {
...
_showBattleScreen: function(enemyData, mode, modeTitle, extraInfo, onAttack, onFlee) {
    ...
    h += '<button onclick="' + onAttack + '" style="background:url(assets/skills/skill_shot_normal.png) center/contain no-repeat;width:72px;height:72px;border:3px solid #c9a040;border-radius:50%;cursor:pointer;"></button>';
    ...
    h += '<div style="display:flex;align-items:center;justify-content:center;gap:8px;margin:4px 0;color:#aaa;font-size:0.7em;"><span style="color:#f44336;">ATK ' + p.stats.attack + '</span> <span style="color:#2196f3;">DEF ' + p.stats.defense + '</span> <span style="color:#ff9800;">AGI ' + p.stats.agility + '</span><img src="assets/hero_skins/' + (Sherwood.Forge && Sherwood.Forge.getActiveSkin ? Sherwood.Forge.getActiveSkin() : 'skin_1_basic') + '.png" style="width:56px;height:56px;border-radius:50%;border:2px solid #c9a040;"></div>';
...
_combatFlee: function() { var r = Sherwood.Combat.flee(); if (r.success) { this._stopBattleMusic(); this._leaveDungeon(); return; } if (r.lose) { this._showDialog('Defeat...', '#f44336'); this._stopBattleMusic(); var self = this; setTimeout(function() { self._leaveDungeon(); }, 1200); return; } this._showDialog('Flee failed! Enemy: -' + r.damage, '#ff9800'); this._showCombatScreen(); },
_handleCombat: function(r) {
    ...
    if (r.win) {
        ...
        this._showDialog((r.crit ? 'CRIT! ' : '') + 'You dealt ' + r.damage + ' damage', r.crit ? '#ff6a00' : '#fff');
        if (r.armorDmg) this._showDialog('Armor broken: ' + r.armorDmg, '#2196f3');
        if (r.enemy && r.enemy.damage) { var self = this; setTimeout(function() { self._showDialog((r.enemyName || 'Enemy') + ' dealt ' + r.enemy.damage + ' damage', '#f44336'); }, 700); }
        this.updateDisplay(); var self = this; setTimeout(function() { self._showCombatScreen(); }, 1000);
    }
},

    // ===== QUESTS =====
    quest: function() { this._playSound('click'); if (!Sherwood.Quests) { this._showPlaceholder('Quests','quests'); return; } var prog = Sherwood.Quests.getProgress(), chapters = Sherwood.Quests.getAllChapters(), energy = Sherwood.Quests.getEnergy(); var cooldown = Sherwood.Quests.isOnCooldown(), cdRemain = Sherwood.Quests.getCooldownRemaining(), accel = Sherwood.Quests.getAccelCost(); var currentChapter = prog.currentChapter || 1, ch = Sherwood.Quests.getChapter(currentChapter); if (!ch) { this._showPlaceholder('Quests','quests'); return; } var completed = prog.completed && prog.completed.indexOf(ch.id) !== -1, unlocked = Sherwood.Quests.isUnlocked(ch.id); var html = '<div style="text-align:center;margin-bottom:8px;"><span style="color:#ff9800;">Energy: '+energy.current+'/'+energy.max+'</span>'; if (cooldown) html += ' <span style="color:#f44336;">CD: '+cdRemain+' min</span>'; html += '</div>'; if (cooldown) html += '<div style="text-align:center;margin-bottom:8px;"><button onclick="SherwoodUI._questAccel()" style="background:#ff9800;border:none;border-radius:6px;padding:6px 14px;color:#fff;cursor:pointer;font-size:0.7em;">Speed up ('+(accel.currency==='free'?'Free':accel.cost+' Gold')+')</button></div>'; html += '<div style="background:url(\'assets/backgrounds/skill_page.jpeg\') center/cover no-repeat;border:2px solid '+(completed?'#4caf50':unlocked?'#c9a040':'#f44336')+';border-radius:12px;padding:14px;margin-bottom:8px;position:relative;overflow:hidden;"><div style="position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,'+(completed?'0.3':'0.5')+');z-index:0;"></div><div style="position:relative;z-index:1;"><div style="color:#e0c080;font-weight:bold;font-size:1.1em;">Chapter '+ch.id+': '+ch.name+'</div><div style="color:#aaa;font-size:0.7em;margin:6px 0;">'+ch.lore+'</div><div style="text-align:center;margin:10px 0;"><img src="assets/all_beasts/'+ch.boss.image+'" style="width:200px;height:200px;object-fit:contain;border:2px solid #f44336;border-radius:12px;" onerror="this.style.display=\'none\'"><div style="color:#f44336;font-weight:bold;margin-top:4px;">'+ch.boss.name+'</div><div style="color:#aaa;font-size:0.65em;">HP '+ch.boss.hp+' | ATK '+ch.boss.atk+' | DEF '+ch.boss.def+'</div></div><div style="display:flex;justify-content:space-between;color:#aaa;font-size:0.7em;"><span>Stages: '+ch.stages+'</span><span>Energy: '+ch.energyCost+'</span></div>'; if (unlocked && !completed && !cooldown) html += '<button onclick="SherwoodUI._startQuest('+ch.id+')" style="width:100%;margin-top:10px;background:#c9a040;border:none;border-radius:8px;padding:10px;color:#000;font-weight:bold;cursor:pointer;">To Battle</button>'; if (completed) html += '<div style="text-align:center;color:#4caf50;font-weight:bold;margin-top:8px;">Completed</div>'; html += '</div></div><div style="display:flex;gap:6px;justify-content:center;">'; if (currentChapter>1) html += '<button onclick="SherwoodUI._prevChapter()" style="background:rgba(255,255,255,0.1);border:1px solid #555;border-radius:6px;padding:6px 14px;color:#fff;cursor:pointer;font-size:0.7em;">Prev</button>'; if (currentChapter<15 && prog.completed.indexOf(currentChapter)!==-1) html += '<button onclick="SherwoodUI._nextChapter()" style="background:rgba(255,255,255,0.1);border:1px solid #555;border-radius:6px;padding:6px 14px;color:#fff;cursor:pointer;font-size:0.7em;">Next</button>'; html += '</div><div style="text-align:center;color:#aaa;font-size:0.6em;margin-top:4px;">Attempts today: '+(Sherwood.Quests.getAttemptsToday?Sherwood.Quests.getAttemptsToday():0)+'</div><div id="quest-info" style="text-align:center;color:#aaa;font-size:0.7em;margin-top:4px;"></div>'; this._openScreen('Quests','quests',html); },
    ...
    _startQuest: function(id) { var r=Sherwood.Quests.startChapter(id),info=document.getElementById('quest-info'); if(!r.success) { if(info) info.textContent=(r.reason||'Error'); if(r.cooldown) this.quest(); return; } this._stopMusic(); this._showQuestBattle(); },
    _showQuestBattle: function() { var b=Sherwood.Quests.getBattle(); if(!b) { this.quest(); return; } var e=b.enemy; this._showBattleScreen({ name:e.name, image:e.image, hp:e.hp, maxHp:e.maxHp },'quest','Chapter '+b.chapter.id+' - Stage '+b.stage+'/'+b.total,'','SherwoodUI._questAttack()','SherwoodUI._questFlee()'); },
    ...
    _handleQuestResult: function(r) {
        ...
        if (r.enemyDead) {
            ...
            this._showDialog('Enemy defeated!','#4caf50');
            this.updateDisplay();
            if (r.chapterComplete) {
                this._showDialog('Chapter complete! +'+r.rewards.exp+'XP +'+r.rewards.gold+'Gold','#ffd700');
            ...
            this._showDialog('Defeat...','#f44336');
            ...
            if (r.enemyDamage) {
                var self = this;
                setTimeout(function() { self._showDialog('Enemy dealt ' + r.enemyDamage + ' damage', '#f44336'); }, 700);
            }
            var self = this;
    ...
    tavern: function() {
        this._playSound('click');
        if (!Sherwood.Tavern) { this._showPlaceholder('Tavern','tavern'); return; }
        ...
        if (active && active.quest && active.row) {
            var q = active.quest;
            html += '<div style="background:rgba(0,0,0,0.6);border:2px solid #c9a040;border-radius:10px;padding:14px;margin-bottom:12px;"><div style="display:flex;align-items:center;gap:10px;"><img src="assets/interface/old_huntsman_bertram.png" style="width:80px;height:80px;object-fit:contain;" onerror="this.style.display=\'none\'"><div style="color:#ffd700;font-weight:bold;">' + active.row.npc + '</div></div><div style="color:#fff;font-size:0.9em;">' + q.name + '</div><div style="color:#aaa;font-size:0.7em;">' + q.desc + '</div><div style="color:#f44336;font-size:0.7em;margin-top:4px;">Enemy: ' + q.enemy.name + ' (HP ' + q.enemy.hp + ')</div><div style="color:#aaa;font-size:0.7em;">Mode: ' + (battleMode ? 'Manual' : 'Auto') + '</div><div style="display:flex;gap:8px;margin-top:8px;">';
            if (battleMode) html += '<button onclick="SherwoodUI._tavernBattle()" style="background:#c9a040;border:none;border-radius:6px;padding:8px 16px;color:#000;cursor:pointer;">Battle</button>';
            else html += '<button onclick="SherwoodUI._tavernAuto()" style="background:#4caf50;border:none;border-radius:6px;padding:8px 16px;color:#fff;cursor:pointer;">Auto</button>';
            html += '<button onclick="SherwoodUI._tavernCancel()" style="background:rgba(244,67,54,0.2);border:1px solid #f44336;border-radius:6px;padding:8px 12px;color:#f44336;cursor:pointer;">Cancel</button></div></div>';
        }
        if (cooldown) html += '<div style="text-align:center;color:#ff9800;padding:12px;background:rgba(0,0,0,0.4);border-radius:8px;margin-bottom:12px;">Cooldown: ' + cdRemain + ' min</div>';
        if (!active && !cooldown) {
            for (var r = 0; r < rows.length; r++) {
                ...
                for (var q = 0; q < row.quests.length; q++) {
                    ...
                    html += '<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:10px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center;"><div><div style="color:#e0c080;font-size:0.85em;">' + quest.name + '</div><div style="color:#aaa;font-size:0.65em;">' + quest.desc + '</div><div style="color:#f44336;font-size:0.6em;">Enemy: ' + quest.enemy.name + '</div><div style="color:#ffd700;font-size:0.6em;">+ ' + quest.reward.exp + 'XP +' + quest.reward.gold + 'G</div></div><button onclick="SherwoodUI._tavernStart(' + r + ',' + q + ')" style="background:#c9a040;border:none;border-radius:4px;padding:6px 12px;color:#000;cursor:pointer;font-size:0.7em;">Take</button></div>';
                }
                html += '</div>';
            }
        }
        html += '<div style="text-align:center;color:#aaa;font-size:0.7em;margin-top:4px;">Total: ' + (Sherwood.Tavern.getCompletedCount ? Sherwood.Tavern.getCompletedCount() : 0) + ' | Next: ' + (battleMode ? 'Battle' : 'Auto') + '</div><div id="tavern-log" style="text-align:center;color:#aaa;font-size:0.7em;margin-top:4px;"></div>';
        this._openScreen('Tavern', 'tavern', html);
    },
    _tavernStart: function(r, q) {
        var result = Sherwood.Tavern.startQuest(r, q);
        if (!result || !result.success) {
            var log = document.getElementById('tavern-log');
            if (log) log.textContent = (result ? result.reason : 'Error');
            return;
        }
    ...
    if (e.hp <= 0) {
        ...
        this._showDialog('Win! +' + r.reward.exp + 'XP +' + r.reward.gold + 'G', '#ffd700');
        this._stopBattleMusic();
        SherwoodUI.updateDisplay();
        var self = this;
        ...
        this._showDialog((crit ? 'CRIT! ' : '') + 'You: ' + dmg + ' dmg', crit ? '#ff6a00' : '#fff');
        if (p.stats.hp <= 0) {
            var self = this;
            setTimeout(function() { self._showDialog('Defeat...', '#f44336'); }, 700);
            ...
            var self = this;
            setTimeout(function() { self._showDialog('Enemy: ' + edmg + ' dmg', '#f44336'); }, 700);
...
_tavernAuto: function() {
    ...
    var log = document.getElementById('tavern-log');
    if (r.completed) { if (log) log.textContent = 'Done! +' + r.reward.exp + 'XP'; SherwoodUI.updateDisplay(); }
    else if (r.failed) { if (log) log.textContent = 'Fail! -' + r.damage + ' HP'; }
...
_tavernCancel: function() {
    ...
    _showPortalBattle: function() { var b=Sherwood.Portal.getCurrentBattle(); if(!b) { this.portal(); return; } var e=b.enemy,ehp=Math.round(((e.hp||e.maxHp)/(e.maxHp||1))*100),tm=Math.floor(b.timeRemaining/60),ts=b.timeRemaining%60,h='<div style="text-align:center;"><div style="color:#aaa;font-size:0.7em;">Time: '+tm+':'+(ts<10?'0':'')+ts+' | Deaths: '+b.deathCount+'</div><div style="color:#e0c080;">'+b.portal.name+' - '+b.level+'/'+b.totalLevels+'</div><div style="margin:12px 0;"><div style="font-size:4em;">'+(e.isBoss?'BOSS':'ENEMY')+'</div><div style="color:#f44336;font-weight:bold;">'+e.name+'</div><div style="background:rgba(0,0,0,0.5);border-radius:6px;height:14px;margin:4px 20%;overflow:hidden;"><div style="background:#f44336;height:100%;width:'+ehp+'%;"></div></div><div style="color:#aaa;">HP '+Math.max(0,e.hp||e.maxHp)+'/'+(e.maxHp||'?')+'</div></div><div style="color:#4caf50;">HP '+Sherwood.getPlayer().stats.hp+'</div><button onclick="SherwoodUI._portalAttack()" style="background:#c9a040;border:none;border-radius:8px;padding:10px 24px;color:#000;font-weight:bold;cursor:pointer;margin:4px;">Attack</button><button onclick="SherwoodUI._portalFlee()" style="margin-top:6px;background:rgba(244,67,54,0.2);border:1px solid #f44336;border-radius:6px;padding:6px 16px;color:#f44336;cursor:pointer;font-size:0.7em;">Flee</button><div id="portal-log" style="color:#aaa;font-size:0.75em;margin-top:8px;"></div></div>'; this._openScreen('Portal','portal',h); },
    _portalAttack: function() { this._handlePortalResult(Sherwood.Portal.portalAttack()); },
    _handlePortalResult: function(r) { if(!r) return; var log=document.getElementById('portal-log'); if(r.portalComplete) { if(log) log.textContent='Portal complete!'; this._stopBattleMusic(); this.updateDisplay(); var self=this; setTimeout(function(){self._playMusic('main_theme');self.portal();},2000); } else if(r.portalFailed) { if(log) log.textContent='Portal failed!'; this._stopBattleMusic(); var self=this; setTimeout(function(){self._playMusic('main_theme');self.portal();},2000); } else if(r.dead&&r.resurrected) { if(log) log.textContent='Death #'+r.deathCount+'! Cost: '+r.cost.cost+' '+r.cost.currency; this.updateDisplay(); this._showPortalBattle(); } else if(r.enemyDead) { if(log) log.textContent='Enemy defeated!'; this.updateDisplay(); if(r.nextEnemy) { var self=this; setTimeout(function(){self._showPortalBattle();},1000); } } else { if(log) log.textContent='-'+r.damage+' | Enemy: -'+(r.enemyDamage||0); this.updateDisplay(); this._showPortalBattle(); } },
    _portalFlee: function() { this._stopBattleMusic(); Sherwood.Portal.fleePortal(); this._playMusic('main_theme'); this.portal(); },

    // ===== RAID =====
    raid: function() { this._playSound('click'); if(!Sherwood.Raid) { this._showPlaceholder('Raid','raid'); return; } if(Sherwood.Raid.isRaidActive()) { this._showRaidBattle(); return; } var raids=Sherwood.Raid.getAvailableRaids(),check=Sherwood.Raid.canJoinRaid(),h=''; for (var i=0;i<raids.length;i++) { var r=raids[i]; h+='<div style="background:rgba(0,0,0,0.5);border:2px solid '+(check.can?'#c9a040':'#f44336')+';border-radius:10px;padding:14px;margin-bottom:8px;text-align:center;"><div style="color:#e0c080;font-weight:bold;">'+r.name+'</div><div style="color:#aaa;">HP '+r.maxHp+' | ATK '+r.attack+' | 3 stages</div>'+(check.can?'<button onclick="SherwoodUI._startRaid('+i+')" style="margin-top:8px;background:#c9a040;border:none;border-radius:6px;padding:8px 20px;color:#000;font-weight:bold;cursor:pointer;">Enter!</button>':'<div style="color:#f44336;">'+check.reason+'</div>')+'</div>'; } this._openScreen('Raid','raid',h||'<div style="color:#aaa;text-align:center;">No raids</div>'); },
    _startRaid: function(i) { this._stopMusic(); Sherwood.Raid.startRaid(i); this._playSound('dungeon_enter'); this._showRaidBattle(); },
    _showRaidBattle: function() { var s=Sherwood.Raid.getRaidStatus(); if(!s) { this.raid(); return; } var stage=s.stage,enemy=null; for (var i=0;i<stage.enemies.length;i++) { if(stage.enemies[i].hp>0) { enemy=stage.enemies[i]; break; } } if(!enemy) { this._raidAttack(); return; } this._showBattleScreen({ name:enemy.name, image:enemy.image, hp:enemy.hp, maxHp:enemy.maxHp },'raid',s.boss.name+' - Stage '+s.stageIndex+'/'+s.totalStages,'','SherwoodUI._raidAttack()','SherwoodUI._raidFlee()'); },
    _raidAttack: function() { this._playHitSounds(); var r=Sherwood.Raid.raidAttack(); if(!r) return; if(r.raidComplete) { this._showDialog('Raid complete! +'+r.rewards.exp+'XP +'+r.rewards.gold+'G','#ffd700'); this._stopBattleMusic(); this.updateDisplay(); var scrolls=Math.random()<0.3?1+Math.floor(Math.random()*3):0; if(scrolls) Sherwood.addResource('scrolls',scrolls); this._pendingRewards={exp:r.rewards.exp,gold:r.rewards.gold,silver:r.rewards.silver,scrolls:scrolls}; this._afterRewardAction=function(){SherwoodUI._playMusic('main_theme');SherwoodUI.raid();}; this._showVictoryScreen(this._pendingRewards); } else if(r.stageComplete) { this._showDialog('Stage complete!','#4caf50'); var self=this; setTimeout(function(){self._showRaidBattle();},1200); } else if(r.playerDead) { this._showDialog('You died!','#f44336'); this._stopBattleMusic(); var scrolls=Math.random()<0.08?1:0; if(scrolls) Sherwood.addResource('scrolls',scrolls); this._pendingRewards={exp:Math.floor(50),silver:Math.floor(100),scrolls:scrolls}; this._afterRewardAction=function(){SherwoodUI._playMusic('main_theme');SherwoodUI.raid();}; this._showDefeatScreen(this._pendingRewards); } else { this._hitEnemyCard(); this._updateEnemyHP(r.enemyHp,r.enemyMaxHp); this._showDialog((r.crit?'CRIT! ':'')+'You dealt '+r.damage+' damage',r.crit?'#ff6a00':'#fff'); if(r.enemyDamage) { var self=this; setTimeout(function(){self._showDialog('Enemy dealt '+r.enemyDamage+' damage','#f44336');},700); } this.updateDisplay(); var self=this; setTimeout(function(){self._showRaidBattle();},1000); } },
    _raidFlee: function() { this._stopBattleMusic(); Sherwood.Raid.fleeRaid(); this._playMusic('main_theme'); this.raid(); },

    // ===== ARENA =====
    arena: function() { this._playSound('click'); if(!Sherwood.Arena) { this._showPlaceholder('Arena','arena'); return; } if(Sherwood.Arena.isInMatch()) { this._showArenaMatch(); return; } var opps=Sherwood.Arena.getOpponents(),stats=Sherwood.Arena.getStats(),h='<div style="text-align:center;margin-bottom:8px;color:#e0c080;">'+stats.rank+' | Wins: '+stats.wins+' | Losses: '+stats.losses+'</div>'; for (var i=0;i<opps.length;i++) { var o=opps[i]; var skinFile = o.skin || 'assets/hero_skins/skin_1_basic.png'; h+='<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:10px;margin-bottom:6px;display:flex;align-items:center;gap:10px;"><img src="'+skinFile+'" style="width:40px;height:40px;border-radius:50%;" onerror="this.src=\'assets/hero_skins/skin_1_basic.png\'"><div style="flex:1;"><div style="color:#fff;">'+o.name+'</div><div style="color:#aaa;font-size:0.7em;">ATK '+o.stats.attack+' DEF '+o.stats.defense+' HP '+o.stats.maxHp+'</div></div><button onclick="SherwoodUI._startArenaMatch('+o.id+')" style="background:#c9a040;border:none;border-radius:4px;padding:6px 12px;color:#000;cursor:pointer;font-size:0.7em;">Fight</button></div>'; } h+='<button onclick="SherwoodUI._refreshArena()" style="width:100%;margin-top:8px;background:rgba(255,255,255,0.1);border:1px solid #666;border-radius:6px;padding:8px;color:#fff;cursor:pointer;">Refresh</button>'; this._openScreen('Arena','arena',h); },
    ...
    _arenaFlee: function() { this._stopBattleMusic(); Sherwood.Arena.fleeMatch(); this._playMusic('main_theme'); this.arena(); },

    // ========== SETTINGS / CHAT / MARKET / PROFILE / BAG ==========
    settings: function() { this._playSound('click'); var p=Sherwood.getPlayer(),nm=p?p.name:'Hunter',h='<div style="background:rgba(0,0,0,0.5);border-radius:10px;padding:16px;margin-bottom:12px;"><div style="color:#fff;margin-bottom:8px;">Name</div><div style="display:flex;gap:8px;"><input id="pni" value="'+nm+'" style="flex:1;background:rgba(255,255,255,0.1);border:1px solid #555;border-radius:6px;padding:8px 12px;color:#fff;font-family:\'Georgia\',serif;font-size:0.9em;"><button onclick="SherwoodUI._changePlayerName()" style="background:#c9a040;border:none;border-radius:6px;padding:8px 16px;color:#000;font-weight:bold;cursor:pointer;">Save</button></div><div id="name-status" style="color:#aaa;font-size:0.7em;margin-top:4px;"></div></div><div style="background:rgba(0,0,0,0.5);border-radius:10px;padding:16px;margin-bottom:12px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><span style="color:#fff;">Sound</span><button onclick="SherwoodUI._toggleSound(' + !this._soundEnabled + ')" style="width:60px;height:30px;background:'+(this._soundEnabled?'#4caf50':'#555')+';border:none;border-radius:15px;cursor:pointer;position:relative;"><span style="position:absolute;top:3px;'+(this._soundEnabled?'right:3px;':'left:3px;')+'width:24px;height:24px;background:#fff;border-radius:50%;transition:0.2s;"></span></button></div><div style="display:flex;justify-content:space-between;align-items:center;"><span style="color:#fff;">Music</span><button onclick="SherwoodUI._toggleMusic(' + !this._musicEnabled + ')" style="width:60px;height:30px;background:'+(this._musicEnabled?'#4caf50':'#555')+';border:none;border-radius:15px;cursor:pointer;position:relative;"><span style="position:absolute;top:3px;'+(this._musicEnabled?'right:3px;':'left:3px;')+'width:24px;height:24px;background:#fff;border-radius:50%;transition:0.2s;"></span></button></div></div><button onclick="SherwoodUI._exitGame()" style="width:100%;background:#f44336;border:none;border-radius:8px;padding:12px;color:#fff;font-weight:bold;font-size:1em;cursor:pointer;">Exit</button>'; this._openScreen('Settings','settings',h); },
    _changePlayerName: function() { var inp=document.getElementById('pni'),st=document.getElementById('name-status'); if(!inp||!st) return; var nm=inp.value.trim(); if(!nm) { st.textContent='Empty name'; st.style.color='#f44336'; return; } var p=Sherwood.getPlayer(); if(p) { p.name=nm; Sherwood.saveGame(); if(Sherwood.Chat) Sherwood.Chat.setUsername(nm); st.textContent='Saved!'; st.style.color='#4caf50'; } },
...
_toggleMusic: function(en) { 
    ...
    _exitGame: function() { if(confirm('Exit to main menu?')) { if(Sherwood.saveGameNow) Sherwood.saveGameNow(); else if(Sherwood.saveGame) Sherwood.saveGame(); this._stopMusic(); this._stopBattleMusic(); if(this._screenLayer) { this._screenLayer.style.display='none'; this._screenLayer.innerHTML=''; } if(this._mainElements) this._mainElements.forEach(function(sel){document.querySelectorAll(sel).forEach(function(el){el.style.display='';});}); this.container.style.background=''; document.getElementById('mainInterface').classList.remove('active'); document.getElementById('loadingScreen').classList.remove('hidden'); this.updateDisplay(); } },
    chat: function() { this._playSound('click'); if(!Sherwood.Chat) { this._showPlaceholder('Chat','chat'); return; } var msgs=Sherwood.Chat.getRecentMessages(50),h=''; for (var i=0;i<msgs.length;i++) { var m=msgs[i]; if(m.isSystem) h+='<div style="color:#888;font-size:0.75em;text-align:center;margin:4px 0;">['+m.time+'] '+m.text+'</div>'; else { var me=m.sender===Sherwood.Chat.getUsername(); h+='<div style="margin-bottom:6px;display:flex;flex-direction:column;align-items:'+(me?'flex-end':'flex-start')+';"><div style="color:#c9a040;font-size:0.65em;">'+m.sender+' <span style="color:#666;">'+m.time+'</span></div><div style="background:'+(me?'rgba(201,168,76,0.2)':'rgba(255,255,255,0.08)')+';border-radius:8px;padding:6px 10px;color:#ddd;font-size:0.8em;max-width:80%;word-break:break-word;">'+m.text+'</div></div>'; } } var c='<div style="display:flex;flex-direction:column;height:100%;"><div id="chat-msgs" style="flex:1;background:rgba(0,0,0,0.5);border-radius:8px;padding:12px;margin-bottom:8px;overflow-y:auto;color:#ccc;font-size:0.85em;min-height:300px;">'+(h||'<div style="color:#666;text-align:center;">Empty</div>')+'</div><div style="display:flex;gap:8px;"><input id="chat-input" placeholder="Message..." style="flex:1;background:rgba(255,255,255,0.1);border:1px solid #555;border-radius:8px;padding:10px;color:#fff;font-family:\'Georgia\',serif;font-size:0.85em;" onkeydown="if(event.key===\'Enter\')SherwoodUI._sendChat()"><button onclick="SherwoodUI._sendChat()" style="background:transparent;border:none;cursor:pointer;padding:0;width:44px;height:44px;"><img src="assets/all_buttons/send_text.png" style="width:100%;height:100%;object-fit:contain;"></button></div></div>'; this._openScreen('Chat','chat',c); setTimeout(function(){var el=document.getElementById('chat-msgs');if(el)el.scrollTop=el.scrollHeight;},100); },
    ...
    market: function() { this._playSound('click'); if(!Sherwood.BlackMarket) { this._showPlaceholder('Market','market'); return; } var items=Sherwood.BlackMarket.getShopItems(),h=''; for (var i=0;i<items.length;i++) { var item=items[i]; h+='<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:10px;margin-bottom:6px;display:flex;align-items:center;gap:10px;"><img src="'+item.icon+'" style="width:44px;height:44px;border-radius:4px;" onerror="this.src=\'assets/interface/labyrinth_of_icons.png\'"><div style="flex:1;"><div style="color:#e0c080;">'+item.name+'</div><div style="color:#aaa;font-size:0.7em;">'+(item.type==='consumable'?'Consumable':item.type==='resource'?'Resource':'Equipment')+'</div></div><div style="text-align:right;"><div style="color:'+(item.currency==='gold'?'#ffd700':'#c0c0c0')+';">'+item.price+' '+(item.currency==='gold'?'Gold':'Silver')+'</div><button onclick="SherwoodUI._buyItem('+item.shopIndex+')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Buy</button></div></div>'; } this._openScreen('Market','market',(h||'<div style="color:#aaa;text-align:center;">No items</div>')+'<div id="market-log" style="text-align:center;color:#aaa;font-size:0.7em;margin-top:8px;"></div>'); },
    _buyItem: function(i) { var r=Sherwood.BlackMarket.buyItem(i),log=document.getElementById('market-log'); if(r.success) { if(log) log.textContent='Bought!'; this.updateDisplay(); } else { if(log) log.textContent=(r.reason||'Error'); } var self=this; setTimeout(function(){self.market();},800); },
    bag: function() { this._playSound('click'); var bag=Sherwood.Bag,items=bag?bag.getItems():[],max=bag?bag.getMaxSlots():10,h=''; for (var i=0;i<max;i++) { var item=items[i]; if(item) { var gc=Sherwood.GradeColors?Sherwood.GradeColors[item.grade]:'#9d9d9d'; h+='<div onclick="SherwoodUI._bagAction('+i+')" style="background:url(\'assets/interface/bag_cell.jpeg\') center/contain no-repeat;background-size:cover;width:60px;height:60px;border:2px solid '+gc+';border-radius:6px;display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;cursor:pointer;padding:4px;"><img src="'+(item.icon||'assets/interface/labyrinth_of_icons.png')+'" style="width:32px;height:32px;object-fit:contain;">'+(item.quantity>1?'<span style="position:absolute;bottom:2px;right:4px;color:#fff;font-size:0.6em;background:rgba(0,0,0,0.7);padding:0 4px;border-radius:4px;">'+item.quantity+'</span>':'')+'</div>'; } else { h+='<div style="background:url(\'assets/interface/bag_cell.jpeg\') center/contain no-repeat;background-size:cover;width:60px;height:60px;border:2px solid #333;border-radius:6px;display:flex;align-items:center;justify-content:center;opacity:0.4;"><span style="color:#555;font-size:0.6em;">empty</span></div>'; } } var expInfo=Sherwood.Bag.getExpansionInfo(),expBtn=expInfo.canExpand?'<button onclick="SherwoodUI._expandBag()" style="margin-top:8px;background:#c9a040;border:none;border-radius:6px;padding:6px 14px;color:#000;cursor:pointer;font-size:0.7em;">Expand +5 ('+expInfo.cost+' Gold)</button>':'<span style="color:#666;font-size:0.6em;">Max for your level</span>'; var c='<div style="color:#aaa;font-size:0.8em;margin-bottom:4px;">'+items.length+'/'+max+'</div>'+expBtn+'<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px;max-width:340px;margin:12px auto 0;">'+h+'</div><div id="bag-info" style="text-align:center;color:#aaa;font-size:0.7em;margin-top:12px;min-height:20px;">Click on item</div>'; this._openScreen('Bag','bag',c); },
    _expandBag: function() { var r=Sherwood.Bag.expandBag(),info=document.getElementById('bag-info'); if(r.success) { if(info) info.textContent='Bag expanded to '+r.newSlots+' slots!'; this.updateDisplay(); } else { if(info) info.textContent=(r.reason||'Error'); } var self=this; setTimeout(function(){self.bag();},800); },
    _bagAction: function(i) {
        ...
        if (item.part === 'ring' || item.part === 'amulet') {
            a += '<button onclick="Sherwood.Bag.equipItem(' + i + ');SherwoodUI.bag();" style="background:#9c27b0;border:none;border-radius:4px;padding:4px 12px;color:#fff;cursor:pointer;margin:0 4px;">Equip</button>';
        } else if (item.part) {
            a += '<button onclick="Sherwood.Bag.equipItem(' + i + ');SherwoodUI.bag();" style="background:#4caf50;border:none;border-radius:4px;padding:4px 12px;color:#fff;cursor:pointer;margin:0 4px;">Equip</button>';
        }
        a += '<button onclick="Sherwood.Bag.sellItem(' + i + ');SherwoodUI.bag();" style="background:#ff9800;border:none;border-radius:4px;padding:4px 12px;color:#fff;cursor:pointer;margin:0 4px;">Sell</button>';
        a += '<button onclick="Sherwood.Bag.discardItem(' + i + ');SherwoodUI.bag();" style="background:#f44336;border:none;border-radius:4px;padding:4px 12px;color:#fff;cursor:pointer;margin:0 4px;">Discard</button>';
        info.innerHTML = '<div style="color:#fff;font-size:0.9em;">' + (item.name || 'Item') + '</div><div style="color:#aaa;font-size:0.7em;">' + (item.grade || 'common') + ' x' + (item.quantity || 1) + '</div><div style="margin-top:6px;">' + a + '</div>';
    },
    profile: function() {
    this._playSound('click');
    var p = Sherwood.getPlayer();
    ...
    var lastTrophy = trophies.length > 0 ? trophies[trophies.length - 1].name : 'No trophies';
    ...
    for (var i = 0; i < allSkins.length; i++) {
        ...
        tabContent += '<div style="color:#e0c080;font-size:0.6em;margin-top:4px;">' + sid.replace('skin_', 'Skin ') + '</div>';
        if (owned) {
            tabContent += isActive ? '<div style="color:#ffd700;font-size:0.55em;">Active</div>' : '<button onclick="SherwoodUI._equipSkinFromProfile(\'' + sid + '\')" style="margin-top:4px;background:#4caf50;border:none;border-radius:4px;padding:2px 8px;color:#fff;cursor:pointer;font-size:0.55em;">Equip</button>';
        } else {
            tabContent += '<div style="color:#888;font-size:0.5em;">Locked</div>';
        }
        tabContent += '</div>';
    }
    tabContent += '</div>';
    ...
    h += '<div style="color:#aaa;">Level ' + p.level + '</div>';
    h += '</div>';
    ...
    if (unlockedList.length > 0) {
        ...
        h += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;"><img src="assets/all_buttons/ranger_skills_button.png" style="width:32px;height:32px;"><span style="color:#e0c080;font-weight:bold;">Talents</span></div>';
...
_showProfileInfo: function(type) { var info=document.getElementById('profile-info'); if(!info) return; if(type==='trophies') { var t=Sherwood.getPlayer().trophies||[]; info.innerHTML=t.length?t.map(function(x){return x.name;}).join(' | '):'No trophies'; } else if(type==='ring') { var r=Sherwood.Bag?Sherwood.Bag.getEquipment().ring:null; info.innerHTML=r?r.name+' (Lvl.'+(r.level||1)+')':'Ring not equipped'; } else if(type==='amulet') { var a=Sherwood.Bag?Sherwood.Bag.getEquipment().amulet:null; info.innerHTML=a?a.name+' (Lvl.'+(a.level||1)+')':'Amulet not equipped'; } },
training: function() { var gb=this._previousScreen==='profile'?'SherwoodUI.profile()':'SherwoodUI.loadHome()'; this._previousScreen=null; this._playSound('click'); var p=Sherwood.getPlayer(),tl=p.trainingLevels||{},stats=['attack','defense','hp','agility'],names={attack:'Attack',defense:'Defense',hp:'Health',agility:'Agility'},colors={attack:'#f44336',defense:'#2196f3',hp:'#4caf50',agility:'#ff9800'},h='<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">'; for (var i=0;i<stats.length;i++) { var s=stats[i],lvl=tl[s]||0; h+='<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:12px;text-align:center;"><div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:4px;"><img src="'+this._statIcons[s]+'" style="width:28px;height:28px;"><span style="color:#e0c080;">'+names[s]+'</span></div><div style="color:#aaa;font-size:0.8em;">Lvl. '+lvl+'/200</div><div style="color:'+colors[s]+';font-size:0.7em;">+'+(s==='hp'?10:s==='agility'?1:2)+' per lvl.</div><button onclick="SherwoodUI._doTraining(\''+s+'\')" style="margin-top:8px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.8em;">Train</button></div>'; } h+='</div><div id="training-log" style="text-align:center;color:#aaa;font-size:0.7em;margin-top:12px;"></div>'; this._openScreen('Training','training',h,gb); },
_doTraining: function(stat) { var p=Sherwood.getPlayer(); if(!p) return; if(!p.trainingLevels) p.trainingLevels={attack:0,defense:0,hp:0,agility:0}; var cur=p.trainingLevels[stat]||0; if(cur>=200) { var log=document.getElementById('training-log'); if(log) log.textContent='Max level!'; return; } var cost=Math.round(10*Math.pow(cur+1,1.15)); if((p.resources.silver||0)<cost) { var log=document.getElementById('training-log'); if(log) log.textContent='Need '+cost+' silver!'; return; } p.resources.silver-=cost; p.trainingLevels[stat]=cur+1; if(Sherwood._recalcStats) Sherwood._recalcStats(); if(Sherwood.saveGame) Sherwood.saveGame(); this.updateDisplay(); this.training(); var log=document.getElementById('training-log'); if(log) log.textContent=stat+' -> '+(cur+1)+' (-'+cost+' silver)'; },

forge: function() {
    var gb = this._previousScreen === 'profile' ? 'SherwoodUI.profile()' : 'SherwoodUI.loadHome()';
    this._previousScreen = null;
    this._playSound('click');
    if (!Sherwood.Forge) { this._showPlaceholder('Forge', 'forge', gb); return; }
    ...
    h += '<div style="margin-bottom:12px;"><div style="color:#e0c080;margin-bottom:4px;">Enhance</div>';
    for (var i = 0; i < enhanceItems.length; i++) {
        var item = enhanceItems[i], idx = items.indexOf(item), lvl = item.enhancement || 0;
        h += '<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:6px;padding:8px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center;"><div><div style="color:#e0c080;font-size:0.8em;">' + item.name + '</div><div style="color:#aaa;font-size:0.6em;">Enhance: +' + lvl + '</div></div><button onclick="SherwoodUI._enhanceItem(' + idx + ')" style="background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Enhance</button></div>';
    }
    h += enhanceItems.length === 0 ? '<div style="color:#aaa;font-size:0.7em;">No items</div>' : '';
    h += '</div>';

    h += '<div style="margin-bottom:12px;"><div style="color:#e0c080;margin-bottom:4px;">Sherwood Arrows</div>';
    h += '<div style="color:#aaa;font-size:0.7em;">In bag: ' + arrowCount + ' pcs.</div>';
    h += '<div style="color:#aaa;font-size:0.65em;">Branches: ' + arrowInfo.branches + ' | Feathers: ' + arrowInfo.feathers + ' | Bones: ' + arrowInfo.bones + '</div>';
    if (arrowInfo.canCraft > 0) {
        h += '<button onclick="SherwoodUI._craftArrow(1)" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Craft 1 arrow</button>';
        if (arrowInfo.canCraft >= 10) h += '<button onclick="SherwoodUI._craftArrow(10)" style="margin-left:4px;background:#ff9800;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Craft 10</button>';
        if (arrowInfo.canCraft >= 100) h += '<button onclick="SherwoodUI._craftArrow(100)" style="margin-left:4px;background:#f44336;border:none;border-radius:4px;padding:4px 10px;color:#fff;cursor:pointer;font-size:0.7em;">Craft all</button>';
    } else {
        h += '<div style="color:#888;font-size:0.6em;">Need: 1 Branch + 1 Feather + 1 Bone</div>';
    }
    h += '</div>';

    h += '<div style="margin-bottom:12px;"><div style="color:#e0c080;margin-bottom:4px;">Rings</div>';
    h += '<div style="color:#aaa;font-size:0.7em;">Level: ' + ringInfo.currentLevel + '/' + ringInfo.maxLevel + ' | Scrolls: ' + ringInfo.scrolls + '</div>';
    if (ringInfo.canCraft) {
        h += '<button onclick="SherwoodUI._craftRing()" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Upgrade (' + ringInfo.cost + ' scrolls)</button>';
    } else {
        h += '<div style="color:#888;font-size:0.6em;">' + (ringInfo.reason || 'Need ' + ringInfo.cost + ' scrolls') + '</div>';
    }
    h += '</div>';

    h += '<div style="margin-bottom:12px;"><div style="color:#e0c080;margin-bottom:4px;">Amulets</div>';
    h += '<div style="color:#aaa;font-size:0.7em;">Level: ' + amuletInfo.currentLevel + '/' + amuletInfo.maxLevel + ' | Scrolls: ' + amuletInfo.scrolls + '</div>';
    if (amuletInfo.canCraft) {
        h += '<button onclick="SherwoodUI._craftAmulet()" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:4px 10px;color:#000;cursor:pointer;font-size:0.7em;">Upgrade (' + amuletInfo.cost + ' scrolls)</button>';
    } else {
        h += '<div style="color:#888;font-size:0.6em;">' + (amuletInfo.reason || 'Need ' + amuletInfo.cost + ' scrolls') + '</div>';
    }
    h += '</div>';

    h += '<div><div style="color:#e0c080;margin-bottom:4px;">Skins</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;">';
    for (var i = 0; i < skins.length; i++) {
        ...
        if (owned) {
            h += isActive ? '<div style="color:#ffd700;font-size:0.6em;">Active</div>' : '<button onclick="SherwoodUI._equipSkin(\'' + skin.id + '\')" style="margin-top:4px;background:#4caf50;border:none;border-radius:4px;padding:3px 8px;color:#fff;cursor:pointer;font-size:0.6em;">Equip</button>';
        } else {
            h += '<div style="color:#aaa;font-size:0.55em;">Ch.' + skin.chapter + ' | ' + skin.cost.ingots + ' ing. ' + skin.cost.scrolls + ' scr. ' + skin.cost.silver + ' sil.</div>';
            h += '<button onclick="SherwoodUI._craftSkin(\'' + skin.id + '\')" style="margin-top:4px;background:#ff9800;border:none;border-radius:4px;padding:3px 8px;color:#fff;cursor:pointer;font-size:0.6em;">Craft</button>';
        }
        h += '</div>';
    }
    h += '</div></div><div id="forge-log" style="text-align:center;color:#aaa;font-size:0.7em;margin-top:8px;"></div>';
    this._openScreen('Forge', 'forge', h, gb);
},

_enhanceItem: function(idx) {
    var r = Sherwood.Forge.enhanceItem(idx);
    var log = document.getElementById('forge-log');
    if (r.enhanced) { if (log) log.textContent = 'Enhanced! +' + r.newLevel; }
    else if (r.broken) { if (log) log.textContent = 'Broken!'; }
    else if (r.failed) { if (log) log.textContent = 'Failed'; }
    else { if (log) log.textContent = (r.reason || 'Error'); }
    this.updateDisplay();
...
_craftArrow: function(count) {
    ...
    var log = document.getElementById('forge-log');
    if (r.success) { if (log) log.textContent = 'Crafted: ' + (r.crafted || 1); }
    else { if (log) log.textContent = (r.reason || 'Error'); }
    this.updateDisplay();
    var self = this; setTimeout(function() { self.forge(); }, 800);
},
...
_craftRing: function() {
    ...
    var log = document.getElementById('forge-log');
    if (r.success) { if (log) log.textContent = 'Ring upgraded to ' + r.newLevel + '!'; }
    else { if (log) log.textContent = (r.reason || 'Error'); }
    this.updateDisplay();
    var self = this; setTimeout(function() { self.forge(); }, 800);
},
...
_craftAmulet: function() {
    ...
    var log = document.getElementById('forge-log');
    if (r.success) { if (log) log.textContent = 'Amulet upgraded to ' + r.newLevel + '!'; }
    else { if (log) log.textContent = (r.reason || 'Error'); }
    this.updateDisplay();
    var self = this; setTimeout(function() { self.forge(); }, 800);
},
...
_craftSkin: function(sid) {
    ...
    var log = document.getElementById('forge-log');
    if (r.success) { if (log) log.textContent = 'Skin crafted!'; }
    else { if (log) log.textContent = (r.reason || 'Error'); }
    this.updateDisplay();
    var self = this; setTimeout(function() { self.forge(); }, 800);
},
...
bestiary: function() { 
    ...
    if(!Sherwood.Bestiary) { this._showPlaceholder('Bestiary','bestiary',gb); return; } 
    var progress=Sherwood.Bestiary.getDiscoveryProgress(),
        zones=['Cursed Thicket','Primeval Swamp','Basalt Mines','Quest','Raid'],
        h='<div style="text-align:center;margin-bottom:8px;color:#aaa;">Discovered: '+progress.discovered+'/'+progress.total+' ('+progress.percent+'%)</div><div style="background:rgba(0,0,0,0.3);border-radius:6px;height:10px;margin-bottom:12px;overflow:hidden;"><div style="background:#c9a040;height:100%;width:'+progress.percent+'%;"></div></div>'; 
