const SherwoodUI = {
    ...
    _scaleGame: function() {
        var container = document.getElementById('game-container'); if (!container) return;
        var windowWidth = window.innerWidth, windowHeight = window.innerHeight;
        var gameWidth = 480, gameHeight = 800;
        var scale = Math.min(windowWidth / gameWidth, windowHeight / gameHeight);
        if (scale < 1) { container.style.transform = 'scale(' + scale + ')'; container.style.transformOrigin = 'top center'; }
        else { container.style.transform = ''; container.style.transformOrigin = ''; }
    },
    ...
    _showPlaceholder: function(title, bgKey, backAction) { this._playSound('click'); this._openScreen(title, bgKey, '<div style="text-align:center;padding:40px 0;"><div style="font-size:3em;margin-bottom:16px;">&#128679;</div><div style="font-size:1.2em;color:#e0c080;margin-bottom:8px;">'+title+'</div><div style="font-size:0.7em;color:#888;">В разработке</div></div>', backAction); },
        ...
        if (rewards.dungeonId && rewards.dungeonLevel) { h += '<div style="margin:10px 0;"><img src="assets/interface/resource_cup_for_completed_tasks.png" style="width:56px;height:56px;object-fit:contain;"><div style="color:#ffd700;font-size:1.2em;font-weight:bold;">+1 Кубок</div><div style="color:#aaa;font-size:0.9em;">Этаж ' + rewards.dungeonLevel + '</div></div>'; }
        if (rewards.exp) h += '<div style="color:#fff;font-size:1.6em;margin:4px 0;">+ ' + rewards.exp + ' Опыта</div>';
        if (rewards.gold) h += '<div style="color:#ffd700;font-size:1.6em;margin:4px 0;"><img src="assets/interface/resource_gold.png" style="width:28px;height:28px;vertical-align:middle;"> + ' + rewards.gold + ' Золота</div>';
        if (rewards.silver) h += '<div style="color:#c0c0c0;font-size:1.6em;margin:4px 0;"><img src="assets/interface/resource_silver.png" style="width:28px;height:28px;vertical-align:middle;"> + ' + rewards.silver + ' Серебра</div>';
        if (rewards.scrolls) h += '<div style="color:#9c27b0;font-size:1.6em;margin:4px 0;">+ ' + rewards.scrolls + ' Скрижалей</div>';
        if (rewards.items && rewards.items.length > 0) { var lootMap = {}; for (var i = 0; i < rewards.items.length; i++) { var item = rewards.items[i]; var key = item.icon + '|' + item.name; if (!lootMap[key]) lootMap[key] = { icon: item.icon, name: item.name, quantity: 0 }; lootMap[key].quantity += item.quantity || 1; } var lootKeys = Object.keys(lootMap); h += '<div style="margin-top:12px;display:flex;flex-wrap:wrap;gap:12px;justify-content:center;">'; for (var k = 0; k < lootKeys.length; k++) { var li = lootMap[lootKeys[k]]; h += '<div style="text-align:center;"><img src="' + li.icon + '" style="width:44px;height:44px;object-fit:contain;"><div style="color:#fff;font-size:1em;">' + li.name + '</div><div style="color:#aaa;font-size:0.85em;">x' + li.quantity + '</div></div>'; } h += '</div>'; }
        h += '</div>';
    ...
    showDungeon: function() {
        ...
        for (var i = 0; i < dungeonList.length; i++) { var dl = dungeonList[i]; var d = dungeons[dl.id]; if (d) { h += '<div style="text-align:center;margin-bottom:24px;"><div style="color:#e0c080;font-size:1.1em;font-weight:bold;margin-bottom:10px;">' + dl.name + '</div><img src="assets/dungeon_tiles/visual_dungeon/' + dl.icon + '" style="width:160px;height:160px;object-fit:contain;display:block;margin:0 auto 10px;">'; if (d.unlocked) { h += '<button onclick="SherwoodUI._showDungeonLevels(\'' + dl.id + '\')" style="background:#c9a040;border:none;border-radius:8px;padding:12px 36px;color:#000;font-weight:bold;cursor:pointer;font-size:0.95em;">Войти</button>'; } else { h += '<div style="color:#f44336;font-size:0.8em;">Заблокировано</div><div style="color:#aaa;font-size:0.65em;">Нужно пройти последний этаж предыдущей подземки на 3 кубка</div>'; } h += '</div>'; } }
    ...
    _showDungeonLevels: function(dungeonId) {
        ...
        for (var lvl = 1; lvl <= 8; lvl++) {
            ...
            h += '<div style="text-align:center;"><div style="display:flex;justify-content:center;gap:2px;margin-bottom:2px;min-height:20px;">';
            for (var c = 0; c < 3; c++) { if (c < cupCount) { h += '<img src="assets/interface/resource_cup_for_completed_tasks.png" style="width:16px;height:16px;object-fit:contain;">'; } else { h += '<div style="width:16px;height:16px;"></div>'; } }
            h += '</div><div onclick="' + (unlocked ? 'SherwoodUI._showDungeonActions(\'' + dungeonId + '\',' + lvl + ')' : '') + '" style="width:56px;height:56px;background-image:url(\'' + img + '\');background-size:cover;background-position:center;border:2px solid ' + (unlocked ? '#c9a040' : '#555') + ';border-radius:8px;cursor:' + (unlocked ? 'pointer' : 'default') + ';display:flex;align-items:center;justify-content:center;position:relative;margin:0 auto;"><span style="position:absolute;bottom:2px;right:4px;font-size:0.6em;color:' + (unlocked ? '#000' : '#888') + ';font-weight:bold;">' + lvl + '</span></div></div>';
        }
        h += '</div>';
        var bgImage = 'assets/dungeon_tiles/visual_dungeon/sherwood_thicket.png';
    ...
    _startDungeon: function(id, level) { 
        ...
        var topBar = "<div style='padding:4px;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;'><button onclick='SherwoodUI._leaveDungeon()' style='background:transparent;border:none;cursor:pointer;padding:0;width:36px;height:36px;'><img src='assets/all_buttons/back.png' style='width:100%;height:100%;object-fit:contain;'></button><div style='color:#70a0e0;font-weight:bold;font-size:0.85em;'>" + (d.id||"") + " " + (d.level||1) + "</div><div style='position:relative;width:300px;height:150px;'><img src='assets/interface/life_scale.png' style='width:100%;height:155%;position:absolute;top:0;left:0;z-index:1;'><div style='position:absolute;top:100px;left:28px;right:28px;bottom:14px;overflow:hidden;z-index:0;'><div style='background:url(assets/interface/life_interface_asset_horizontal_progress_bar.jpeg) left/auto 100%;height:100%;width:" + hpPct + "%;transition:width 0.5s;'></div></div><span style='position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-size:0.7em;z-index:2;text-shadow:0 0 6px #000;font-weight:bold;'>" + hp + "</span></div></div><div style='background:rgba(0,0,0,0.5);padding:3px;text-align:center;flex-shrink:0;'><span style='font-size:10px;color:#aaa;'>" + (d.monstersKilled||0) + "/" + (d.totalMonsters||0) + " | " + (d.monstersKilled >= d.totalMonsters ? "EXIT OPEN" : "KILL ALL") + "</span></div>";
    ...
    _hitEnemyCard: function() { var card = document.getElementById('enemy-card'); if (!card) return; card.style.transition = 'transform 0.1s, filter 0.15s'; card.style.transform = 'translateX(2px) rotate(0.5deg) scale(0.95)'; card.style.filter = 'brightness(1.3) saturate(2) hue-rotate(-10deg)'; setTimeout(function() { card.style.transform = ''; card.style.filter = ''; }, 200); },
    ...
    _arenaDefeat: function() { this._stopMusic(); Sherwood.Arena._losses++; var player = Sherwood.getPlayer(); player.stats.hp = Math.max(1, Math.floor(player.stats.maxHp * 0.2)); Sherwood.saveGame(); this._currentArenaOpponents = null; this._pendingRewards = { exp: 20, silver: 50 }; this._afterRewardAction = function() { SherwoodUI._playMusic('main_theme'); SherwoodUI.arena(); }; this._showDefeatScreen(this._pendingRewards); },
