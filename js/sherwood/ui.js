...
const SherwoodUI = {
        ...
        for (var y = 0; y < size; y++) {
            ...
            for (var x = 0; x < size; x++) {
                ...
                if (cell.open) {
                    ...
                    else if (cell.exit) { content = '<img src="assets/interface/exit_completion_dungeon.png" style="width:80%;height:80%;object-fit:contain;">'; border = '#4caf50'; }
    ...
    _showCombatScreen: function() {
        ...
        h += '<div style="background:rgba(0,0,0,0.5);border-radius:6px;height:14px;width:160px;overflow:hidden;position:relative;">';
        h += '<div id="enemy-hp-bar" class="hp-burn" style="background:#f44336;height:100%;width:' + ehp + '%;transition:0.3s;"></div>';
        h += '<span id="enemy-hp-text" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-size:0.55em;">' + b.enemyHp + '/' + b.enemyMaxHp + '</span></div></div>';
        h += '<div style="color:#4caf50;margin-top:8px;">❤️ ' + b.playerHp + '/' + b.playerMaxHp + '</div>';
    ...
    _handleCombat: function(r) {
        if (!r) return;
        if (r.win) { this._addCombatLog('🏆 Победа! +' + r.exp + 'XP +' + r.gold + '🪙', '#ffd700'); if (Sherwood.Dungeon&&Sherwood.Dungeon.killMonster) Sherwood.Dungeon.killMonster(); this.updateDisplay(); var self = this; setTimeout(function() { self._renderDungeon(); }, 1500); }
        else if (r.lose) { this._addCombatLog('💀 Поражение...', '#f44336'); var self = this; setTimeout(function() { self._leaveDungeon(); }, 1500); }
        else { this._playHitAnimation(); this._updateEnemyHP(r.enemyHp, r.enemyMaxHp); var msg = (r.crit?'💥 КРИТ! ':'⚔️ ')+'-'+r.damage; if (r.armorDmg) msg += ' 🛡️-'+r.armorDmg; this._addCombatLog(msg, r.crit?'#ff6a00':'#fff'); if (r.poison) this._addCombatLog('🧪 Враг отравлен!', '#4caf50'); if (r.stun) this._addCombatLog('💫 Враг оглушён!', '#ffd700'); if (r.enemy&&r.enemy.damage) this._addCombatLog('💢 Враг атакует: -'+r.enemy.damage, '#f44336'); this.updateDisplay(); this._showCombatScreen(); }
    },
    ...
    quest: function() {
        ...
        if (cooldown) html += '<div style="text-align:center;margin-bottom:8px;"><button onclick="SherwoodUI._questAccel()" style="background:#ff9800;border:none;border-radius:6px;padding:6px 14px;color:#fff;cursor:pointer;font-family:\'Georgia\',serif;font-size:0.7em;">⚡ Ускорить (' + (accel.currency==='free'?'Бесплатно':accel.cost+' 🪙') + ')</button></div>';
        ...
        html += '<div style="position:relative;z-index:1;"><div style="color:#e0c080;font-weight:bold;font-size:1.1em;">Глава ' + ch.id + ': ' + ch.name + '</div>';
        html += '<div style="color:#aaa;font-size:0.7em;margin:6px 0;">' + ch.lore + '</div>';
        html += '<div style="text-align:center;margin:10px 0;"><img src="assets/all_beasts/' + ch.boss.image + '" style="width:200px;height:200px;object-fit:contain;border:2px solid #f44336;border-radius:12px;" onerror="this.style.display=\'none\'"><div style="color:#f44336;font-weight:bold;margin-top:4px;">' + ch.boss.name + '</div><div style="color:#aaa;font-size:0.65em;">❤️ ' + ch.boss.hp + ' | ⚔️ ' + ch.boss.atk + ' | 🛡️ ' + ch.boss.def + '</div></div>';
        ...
        if (unlocked && !completed && !cooldown) html += '<button onclick="SherwoodUI._startQuest(' + ch.id + ')" style="width:100%;margin-top:10px;background:#c9a040;border:none;border-radius:8px;padding:10px;color:#000;font-weight:bold;cursor:pointer;font-family:\'Georgia\',serif;">⚔️ В бой</button>';
        ...
        html += '</div><div style="text-align:center;color:#aaa;font-size:0.6em;margin-top:4px;">Попыток сегодня: ' + Sherwood.Quests.getAttemptsToday() + '</div>';
    ...
    _prevChapter: function() { var p = Sherwood.getPlayer(); var cur = p.questProgress.currentChapter || 1; if (cur > 1) p.questProgress.currentChapter = cur - 1; Sherwood.saveGame(); this.quest(); },
    _nextChapter: function() { var p = Sherwood.getPlayer(); var cur = p.questProgress.currentChapter || 1; if (cur < 15 && p.questProgress.completed.indexOf(cur) !== -1) p.questProgress.currentChapter = cur + 1; Sherwood.saveGame(); this.quest(); },
    ...
    _startQuest: function(id) { var r = Sherwood.Quests.startChapter(id); var info = document.getElementById('quest-info'); if (!r.success) { if (info) info.textContent = '❌ ' + (r.reason||'Ошибка'); if (r.cooldown) this.quest(); return; } this._showQuestBattle(); },
    ...
    _showQuestBattle: function() {
        ...
        var h = '<div style="text-align:center;"><div style="color:#aaa;font-size:0.7em;">Глава ' + b.chapter.id + ' — Этап ' + b.stage + '/' + b.total + '</div>';
        h += '<div style="margin:12px 0;position:relative;display:inline-block;">';
        h += '<img src="assets/all_beasts/' + e.image + '" id="enemy-card" style="width:200px;height:200px;object-fit:contain;position:relative;z-index:0;border-radius:12px;" onerror="this.style.display=\'none\'">';
        ...
        h += '</div><div style="color:#f44336;font-weight:bold;">' + e.name + '</div>';
        h += '<div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-top:4px;"><div style="background:rgba(0,0,0,0.5);border-radius:6px;height:12px;width:140px;overflow:hidden;position:relative;"><div id="enemy-hp-bar" class="hp-burn" style="background:#f44336;height:100%;width:' + ehp + '%;transition:0.3s;"></div><span id="enemy-hp-text" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-size:0.55em;">' + e.hp + '/' + e.maxHp + '</span></div></div>';
    ...
    tavern: function() {
        ...
        var rows = Sherwood.Tavern.getAvailableRows(), active = Sherwood.Tavern.getCurrentQuest();
        var cooldown = Sherwood.Tavern.isOnCooldown(), cdRemain = Sherwood.Tavern.getCooldownRemaining(), battleMode = Sherwood.Tavern.getBattleMode();
        var html = '';
        if (active) {
            var q = active.quest;
            html += '<div style="background:rgba(0,0,0,0.6);border:2px solid #c9a040;border-radius:10px;padding:14px;margin-bottom:12px;"><div style="color:#ffd700;font-weight:bold;">⚔️ ' + q.row.npc + '</div><div style="color:#fff;font-size:0.9em;">' + q.quest.name + '</div><div style="color:#aaa;font-size:0.7em;">' + q.quest.desc + '</div><div style="color:#f44336;font-size:0.7em;margin-top:4px;">Противник: ' + q.quest.enemy.name + ' (❤️' + q.quest.enemy.hp + ')</div><div style="color:#aaa;font-size:0.7em;margin-top:4px;">Режим: ' + (battleMode?'⚔️ Ручной бой':'⚡ Автобой') + '</div><div style="display:flex;gap:8px;margin-top:8px;">';
            if (battleMode) html += '<button onclick="SherwoodUI._tavernBattle()" style="background:#c9a040;border:none;border-radius:6px;padding:8px 16px;color:#000;cursor:pointer;font-family:\'Georgia\',serif;">⚔️ В бой</button>';
            else html += '<button onclick="SherwoodUI._tavernAuto()" style="background:#4caf50;border:none;border-radius:6px;padding:8px 16px;color:#fff;cursor:pointer;font-family:\'Georgia\',serif;">⚡ Автобой</button>';
            html += '<button onclick="SherwoodUI._tavernCancel()" style="background:rgba(244,67,54,0.2);border:1px solid #f44336;border-radius:6px;padding:8px 12px;color:#f44336;cursor:pointer;font-family:\'Georgia\',serif;">Отменить</button></div></div>';
        }
        ...
        if (!active && !cooldown) {
            for (var r = 0; r < rows.length; r++) {
                var row = rows[r];
                html += '<div style="margin-bottom:12px;"><div style="color:#e0c080;font-weight:bold;margin-bottom:4px;">📜 ' + row.name + ' <span style="color:#aaa;font-size:0.7em;">— ' + row.npc + '</span></div>';
    ...
    _showTavernBattle: function() {
        var active = Sherwood.Tavern.getCurrentQuest(); if (!active) { this.tavern(); return; }
        var e = active.quest.enemy, p = Sherwood.getPlayer(), ehp = Math.round((e.hp/(e.hp||1))*100), php = Math.round((p.stats.hp/p.stats.maxHp)*100);
        var h = '<div style="text-align:center;"><div style="color:#aaa;font-size:0.7em;">' + active.row.npc + ' — ' + active.quest.name + '</div>';
        h += '<div style="margin:12px 0;"><img src="assets/all_beasts/' + e.image + '" style="width:180px;height:180px;object-fit:contain;border:2px solid #f44336;border-radius:12px;" onerror="this.style.display=\'none\'"><div style="color:#f44336;font-weight:bold;">' + e.name + '</div>';
        h += '<div style="background:rgba(0,0,0,0.5);border-radius:6px;height:14px;margin:4px 20%;overflow:hidden;"><div style="background:#f44336;height:100%;width:' + ehp + '%;"></div></div><div style="color:#aaa;font-size:0.7em;">❤️ ' + e.hp + '</div></div>';
        h += '<div style="color:#4caf50;">❤️ ' + p.stats.hp + '/' + p.stats.maxHp + '</div>';
        h += '<button onclick="SherwoodUI._tavernBattleAttack()" style="margin-top:10px;background:#c9a040;border:none;border-radius:8px;padding:10px 24px;color:#000;font-weight:bold;cursor:pointer;font-family:\'Georgia\',serif;">⚔️ Атака</button>';
        h += '<div id="tavern-battle-log" style="color:#aaa;font-size:0.75em;margin-top:8px;"></div></div>';
        this._openScreen('⚔️ Таверна', 'tavern', h);
    },
    ...
    _tavernBattleAttack: function() {
        ...
        e.hp -= dmg; var log = document.getElementById('tavern-battle-log');
        if (e.hp <= 0) { var r = Sherwood.Tavern.completeQuest(); if (log) log.textContent = '🏆 Победа! +' + r.reward.exp + 'XP +' + r.reward.gold + '🪙'; this.updateDisplay(); var self = this; setTimeout(function() { self.tavern(); }, 1500); }
        else { var edmg = Math.max(1, Math.floor((e.atk*e.atk)/(e.atk+p.stats.defense))); p.stats.hp = Math.max(0, p.stats.hp-edmg); if (log) log.textContent = (crit?'💥 КРИТ! ':'') + '-' + dmg + ' | Враг: -' + edmg;
            if (p.stats.hp <= 0) { if (log) log.textContent += ' | 💀 Поражение!'; Sherwood.Tavern.failQuest(); p.stats.hp = 1; var self = this; setTimeout(function() { self.tavern(); }, 1500); }
            else this._showTavernBattle(); }
        this.updateDisplay();
    },

    _tavernAuto: function() { var r = Sherwood.Tavern.autoBattle(); var log = document.getElementById('tavern-log'); if (r.completed) { if (log) log.textContent = '🎉 Выполнено! +' + r.reward.exp + 'XP'; this.updateDisplay(); } else if (r.failed) { if (log) log.textContent = '❌ Неудача! -' + r.damage + ' HP'; } var self = this; setTimeout(function() { self.tavern(); }, 800); },
    ...
    daily: function() {
        ...
        var p = Sherwood.getPlayer(), currentChapter = p.questProgress ? (p.questProgress.currentChapter || 1) : 1;
        var chapterQuests = Sherwood.Daily.getChapterQuests(currentChapter), chapterCompleted = p.daily ? (p.daily.chapterCompleted || []) : [];
        var html = '';
        var t1b = (!SherwoodUI._dailyTab||SherwoodUI._dailyTab===1)?'#c9a040':'rgba(255,255,255,0.1)', t1c = (!SherwoodUI._dailyTab||SherwoodUI._dailyTab===1)?'#000':'#fff';
        var t2b = (SherwoodUI._dailyTab===2)?'#c9a040':'rgba(255,255,255,0.1)', t2c = (SherwoodUI._dailyTab===2)?'#000':'#fff';
        ...
        if (!SherwoodUI._dailyTab || SherwoodUI._dailyTab === 1) {
            for (var i = 0; i < dailyQuests.length; i++) { var q = dailyQuests[i], claimed = dailyCompleted.indexOf(q.id)!==-1;
                html += '<div style="background:rgba(0,0,0,0.5);border:1px solid '+(q.completed?(claimed?'#4caf50':'#ffd700'):'#555')+';border-radius:8px;padding:10px;margin-bottom:6px;"><div style="color:#e0c080;font-size:0.85em;">'+q.name+'</div><div style="color:#aaa;font-size:0.65em;">'+q.desc+'</div><div style="background:rgba(0,0,0,0.3);border-radius:4px;height:8px;margin:6px 0;overflow:hidden;"><div style="background:'+(q.completed?'#4caf50':'#c9a040')+';height:100%;width:'+Math.round((q.progress||0)/q.target*100)+'%;"></div></div><div style="color:#aaa;font-size:0.6em;">'+(q.progress||0)+'/'+q.target+' | 🏆 +'+q.reward.gold+'🪙 +'+q.reward.exp+'XP</div>';
                if (q.completed && !claimed) html += '<button onclick="SherwoodUI._claimDaily('+i+')" style="margin-top:4px;background:#4caf50;border:none;border-radius:4px;padding:4px 10px;color:#fff;cursor:pointer;font-size:0.6em;">Забрать</button>';
                if (claimed) html += '<span style="color:#4caf50;font-size:0.6em;">✅ Получено</span>'; html += '</div>'; }
        } else {
            for (var j = 0; j < chapterQuests.length; j++) { var q = chapterQuests[j], claimed = chapterCompleted.indexOf(q.id)!==-1;
                html += '<div style="background:rgba(0,0,0,0.5);border:1px solid '+(q.completed?(claimed?'#4caf50':'#ffd700'):'#555')+';border-radius:8px;padding:10px;margin-bottom:6px;"><div style="color:#e0c080;font-size:0.85em;">'+q.name+'</div><div style="color:#aaa;font-size:0.65em;">'+q.desc+'</div><div style="background:rgba(0,0,0,0.3);border-radius:4px;height:8px;margin:6px 0;overflow:hidden;"><div style="background:'+(q.completed?'#4caf50':'#c9a040')+';height:100%;width:'+Math.round((q.progress||0)/q.target*100)+'%;"></div></div><div style="color:#aaa;font-size:0.6em;">'+(q.progress||0)+'/'+q.target+' | 🏆 +'+q.reward.gold+'🪙 +'+q.reward.exp+'XP</div>';
                if (q.completed && !claimed) html += '<button onclick="SherwoodUI._claimChapter('+currentChapter+','+j+')" style="margin-top:4px;background:#4caf50;border:none;border-radius:4px;padding:4px 10px;color:#fff;cursor:pointer;font-size:0.6em;">Забрать</button>';
                if (claimed) html += '<span style="color:#4caf50;font-size:0.6em;">✅ Получено</span>'; html += '</div>'; }
        }
    ...
    _claimDaily: function(i) { var r = Sherwood.Daily.claimDailyReward(i); var log = document.getElementById('daily-log'); if (r.success) { if (log) log.textContent = '✅ Награда получена!'; this.updateDisplay(); } else { if (log) log.textContent = '❌ ' + r.reason; } var self = this; setTimeout(function() { self.daily(); }, 800); },
    _claimChapter: function(ch, i) { var r = Sherwood.Daily.claimChapterReward(ch, i); var log = document.getElementById('daily-log'); if (r.success) { if (log) log.textContent = '✅ Награда получена!'; this.updateDisplay(); } else { if (log) log.textContent = '❌ ' + r.reason; } var self = this; setTimeout(function() { self.daily(); }, 800); },

    // ========== ОСТАЛЬНЫЕ РЕЖИМЫ ==========
    bag: function() {
        this._playSound('click'); var bag = Sherwood.Bag, items = bag?bag.getItems():[], max = bag?bag.getMaxSlots():10, h = '';
        for (var i = 0; i < max; i++) { var item = items[i];
            ...
            else { h += '<div style="background:url(\'assets/interface/bag_cell.jpeg\') center/contain no-repeat;background-size:cover;width:60px;height:60px;border:2px solid #333;border-radius:6px;display:flex;align-items:center;justify-content:center;opacity:0.4;"><span style="color:#555;font-size:0.6em;">пусто</span></div>'; } }
