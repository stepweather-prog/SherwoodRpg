...
Sherwood.Bag = {
    _inventory: [], _equipment: { amulet: null, ring: null }, _maxSlots: 10, _expansionLevel: 0,

    init: function() {
        var player = Sherwood.getPlayer(); if (!player) return;
        ...
        if (item.id) {
            for (var i = 0; i < this._inventory.length; i++) {
                ...
                if (existing.id === item.id && (existing.quantity || 1) < maxStack) {
                    ...
                    quantity -= add;
                    if (quantity <= 0) { this._save(); return true; }
                }
            }
        }
        while (quantity > 0) {
            ...
            var newItem = Object.assign({}, item, { quantity: addQty, maxStack: maxStack });
    ...
    removeItem: function(index, quantity) {
        ...
        var item = this._inventory[index]; if (!item) return false;
        if (item.quantity && item.quantity > quantity) { item.quantity -= quantity; this._save(); return true; }
        this._inventory.splice(index, 1); this._save(); return true;
    },

    equipItem: function(index) {
        if (index < 0 || index >= this._inventory.length) return false;
        var item = this._inventory[index]; if (!item || !item.part) return false;
        var part = item.part, oldItem = this._equipment[part];
        if (oldItem) { if (this.isFull()) return false; this._inventory.push(oldItem); }
    ...
    discardItem: function(index) { if (index < 0 || index >= this._inventory.length) return false; this._inventory.splice(index, 1); this._save(); return true; },

    sellItem: function(index) {
        if (index < 0 || index >= this._inventory.length) return false;
        var item = this._inventory[index]; if (!item) return false;
        var price = (item.sellPrice || 5) * (item.quantity || 1);
        Sherwood.addResource('silver', price);
        this._inventory.splice(index, 1); this._save();
        return { success: true, price: price };
    },

    _save: function() {
        var player = Sherwood.getPlayer(); if (!player) return;
        player.inventory = this._inventory; player.equipment = this._equipment;
        player.bagSize = this._maxSlots; player.bagExpansion = this._expansionLevel;
        Sherwood.saveGame();
    }
};
