...
Sherwood.Portal = {
    ...
    init: function() {
        ...
        if (!player) return;
        if (!player.portal) player.portal = { completed: [], deathCounts: {} };
    },
    ...
    enterPortal: function(portalId) {
        ...
        for (var i = 0; i < portal.enemies.length; i++) portal.enemies[i].maxHp = portal.enemies[i].hp;
        this._startTimer();
        return { success: true, portal: portal, enemies: portal.enemies, timeLimit: this._timeRemaining };
    },
    ...
    _startTimer: function() {
        ...
        this._timerInterval = setInterval(function() {
            ...
            if (Sherwood.Portal._timeRemaining <= 0) Sherwood.Portal._exitPortal(false);
        }, 1000);
    },
    ...
    _completePortal: function() {
        ...
        var mult = (player.portal.completed && player.portal.completed.indexOf(portal.id) !== -1) ? 0.3 : 1;
    ...
    _exitPortal: function(success) {
        if (this._timerInterval) clearInterval(this._timerInterval);
        this._inPortal = false; this._currentPortal = null;
