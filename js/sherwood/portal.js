...
Sherwood.Portal = {
    ...
    PORTALS: [
        { id: 1, name: 'Портал Нашествия', icon: '🌀', bg: 'assets/backgrounds/portal_1.jpeg', requiredChapter: 5, boss: { name: 'Жнец-Полководец', image: 'the_reaper_commander.png', hp: 3000, attack: 120, defense: 60, exp: 500, gold: 400 }, guard: { name: 'Матка Лесных Короедов', image: 'the_hive_mother.png', hp: 2000, attack: 95, defense: 46, exp: 300, gold: 250 }, rewards: { gold: 500, exp: 800, silver: 1200 } },
        { id: 2, name: 'Портал Черных Пауков', icon: '🕷️', bg: 'assets/backgrounds/portal_2.png', requiredChapter: 7, boss: { name: 'Ткачиха Мрака', image: 'the_dark_weaver.png', hp: 3500, attack: 135, defense: 65, exp: 600, gold: 480 }, guard: { name: 'Проклятый Король Разбойников', image: 'the_cursed_outlaw_king.png', hp: 2300, attack: 105, defense: 50, exp: 350, gold: 300 }, rewards: { gold: 650, exp: 1000, silver: 1500 } },
        { id: 3, name: 'Портал Увядания', icon: '🥀', bg: 'assets/backgrounds/portal_3.png', requiredChapter: 9, boss: { name: 'Истлевший Титан', image: 'the_decayed_titan.png', hp: 4000, attack: 150, defense: 70, exp: 700, gold: 560 }, guard: { name: 'Древний Хранитель Склепа', image: 'ancient_crypt_warden.png', hp: 2600, attack: 115, defense: 55, exp: 400, gold: 350 }, rewards: { gold: 800, exp: 1200, silver: 1800 }, trophy: { id: 'portal_3', name: 'Окровавленный Клык Волка', bonus: { attack: 15, defense: 15, hp: 15 }, icon: 'assets/all_trophies/portal_trophies/1_wolf_fang.png' } },
        { id: 4, name: 'Портал Цепей', icon: '⛓️', bg: 'assets/backgrounds/portal_1.jpeg', requiredChapter: 10, boss: { name: 'Вечный Узник', image: 'the_eternal_prisoner.png', hp: 4500, attack: 165, defense: 75, exp: 800, gold: 640 }, guard: { name: 'Эхо Трех Порталов', image: 'echo_of_the_triumvirate.png', hp: 3000, attack: 130, defense: 60, exp: 450, gold: 400 }, rewards: { gold: 950, exp: 1400, silver: 2100 }, trophy: { id: 'portal_4', name: 'Сердце Ненасытного Тритона', bonus: { attack: 30, defense: 30, hp: 30 }, icon: 'assets/all_trophies/portal_trophies/2_heart_of_the_Insatiable_triton.png' } },
        { id: 5, name: 'Портал Ликантропов', icon: '🐺', bg: 'assets/backgrounds/portal_2.png', requiredChapter: 12, boss: { name: 'Кровавый Вожак', image: 'the_blood_alpha.png', hp: 5000, attack: 180, defense: 80, exp: 900, gold: 720 }, guard: { name: 'Палач Священного Древа', image: 'sacred_tree_executioner.png', hp: 3500, attack: 145, defense: 68, exp: 500, gold: 450 }, rewards: { gold: 1100, exp: 1600, silver: 2400 }, trophy: { id: 'portal_5', name: 'Изумрудный Осколок Исполина', bonus: { attack: 50, defense: 50, hp: 50 }, icon: 'assets/all_trophies/portal_trophies/3_emerald_shard_of_the_giant.png' } },
    ...
    // Сколько токенов портала нужно на вход
    getRequiredTokens: function(id) {
        return id * 3;
    },
    ...
    getPortal: function(id) { for (var i = 0; i < this.PORTALS.length; i++) { if (this.PORTALS[i].id === id) return this.PORTALS[i]; } return null; },
    ...
    getPortalStatus: function(id) {
        ...
        return { id: portal.id, name: portal.name, icon: portal.icon, unlocked: this.isPortalUnlocked(id), completed: this.isPortalCompleted(id), difficulty: this.getPortalDifficulty(id), requiredChapter: portal.requiredChapter };
    },
    ...
    countTokens: function() {
        ...
        for (var i = 0; i < items.length; i++) {
            if (items[i].id === 'portal_token_1') total += (items[i].quantity || 0);
        }
        return total;
    },

    // Списывает portalToken1 из сумки
    _spendTokens: function(amount) {
        if (typeof Sherwood.Bag === 'undefined') return false;
        var items = Sherwood.Bag.getItems();
        var toRemove = amount;
        for (var i = items.length - 1; i >= 0 && toRemove > 0; i--) {
            if (items[i].id === 'portal_token_1') {
    ...
    enterPortal: function(id) {
        ...
        var required = this.getRequiredTokens(id);
        if (!this._spendTokens(required)) {
            return { success: false, reason: 'Не удалось списать токены' };
        }
    ...
    _startTimer: function() {
        ...
        this._timerInterval = setInterval(function() {
            self._timeRemaining--;
            if (self._timeRemaining <= 0) { self._exitPortal(false); UI._showToast('⏰ Время вышло! Ты покинул портал.'); }
        }, 1000);
    },
    ...
    _handleDeath: function() {
        this._deathCount++;
        var player = Sherwood.getPlayer();
        if (this._deathCount > 5) { this._exitPortal(false); return { dead: true, portalFailed: true, reason: 'Слишком много смертей. Ты изгнан из портала.' }; }
        var cost;
        if (this._deathCount <= 2) cost = { cost: this._deathCount * 2500, currency: 'silver' };
        else cost = { cost: 50 + (this._deathCount - 2) * 50, currency: 'gold' };
        if (cost.cost > 0 && (player.resources[cost.currency] || 0) < cost.cost) { this._exitPortal(false); return { dead: true, portalFailed: true, reason: 'Недостаточно ' + cost.currency + ' для воскрешения' }; }
    ...
    _completePortal: function() {
        ...
        if (player.portal.completed.indexOf(portal.id) === -1) player.portal.completed.push(portal.id);
        if (firstTime && portal.trophy && typeof Sherwood.addTrophy === 'function') Sherwood.addTrophy(portal.trophy.id, portal.trophy.name, portal.trophy.bonus, portal.trophy.icon, 'portal');
    ...
    showUI: function() {
        if (typeof UI === 'undefined') {
            if (typeof showGenericScreen === 'function') showGenericScreen('Порталы', '🌀');
            return;
        }
        UI._playSound('click');
        ...
        for (var i = 0; i < allPortals.length; i++) {
            ...
            var requiredTokens = this.getRequiredTokens(portal.id);
            var canEnter = tokens >= requiredTokens;
