...
Sherwood.Training = {
    ...
    showUI: function() {
        ...
        for (var i = 0; i < stats.length; i++) {
            var s = stats[i];
            var info = this.getStatInfo(s);

            h += '<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:12px;margin-bottom:8px;">';
            h += '<div style="display:flex;align-items:center;gap:10px;">';
            h += '<img src="' + icons[s] + '" style="width:40px;height:40px;object-fit:contain;">';
            h += '<div style="flex:1;">';
            h += '<div style="color:' + colors[s] + ';font-weight:bold;">' + names[s] + '</div>';
            h += '<div style="color:#aaa;font-size:0.8em;">Уровень: ' + info.level + '/1000</div>';
            h += '<div style="color:#888;font-size:0.7em;">+' + info.bonus + ' за уровень</div>';
            h += '</div>';
            h += '</div>';

            if (info.isMax) {
                h += '<div style="color:#4caf50;font-weight:bold;text-align:center;margin-top:4px;">✅ МАКСИМУМ</div>';
            } else {
                var currencyIcon = info.currency === 'gold' ? '💰' : '🥈';
                h += '<div style="color:#e0c080;font-size:0.75em;text-align:center;margin:4px 0;">Следующая тренировка: ' + currencyIcon + ' ' + info.cost + ' ' + info.currencyName + '</div>';
                h += '<button onclick="Sherwood.Training._trainFromUI(\'' + s + '\')" class="btn btn-gold" style="width:100%;padding:6px;font-size:0.85em;">⬆ Тренировать</button>';
            }
            h += '</div>';
        }
